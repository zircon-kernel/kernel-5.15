...
enum P61_DEBUG_LEVEL { P61_DEBUG_OFF, P61_FULL_DEBUG };
...
struct p61_dev {
	...
	struct device
		*nfcc_device; /*nfcc driver handle for driver to driver comm */
...
static void p61_stop_throughput_measurement(unsigned int type, int no_of_bytes)
{
	if (type == READ_THROUGH_PUT) {
		...
		p61_through_put_t.total_through_put_rtime +=
			(long)ktime_to_ns(
				ktime_sub(p61_through_put_t.rstop_tv,
		...
		p61_through_put_t.total_through_put_wtime +=
			(long)ktime_to_ns(
				ktime_sub(p61_through_put_t.wstop_tv,
		...
		printk(KERN_ALERT
		       " p61_stop_throughput_measurement: wrong type = %u",
		       type);
	}
}
...
static int p61_dev_open(struct inode *inode, struct file *filp)
{
	...
	if (p61_dev != NULL && p61_dev->nfcc_data == NULL) {
		struct device *nfc_dev = bus_find_device_by_name(
			&i2c_bus_type, NULL, p61_dev->nfcc_name);
		if (nfc_dev) {
			...
			P61_ERR_MSG(
				"%s: cannot find NFC controller '%s' skip\n",
			  ...
			  unsigned long arg)
{
	...
	case ESE_PERFORM_COLD_RESET:
		...
		P61_DBG_MSG(KERN_ALERT " ESE_PERFORM_COLD_RESET ret: %d exit",
			    ret);
		break;
	case PERFORM_RESET_PROTECTION:
		P61_DBG_MSG(KERN_ALERT " PERFORM_RESET_PROTECTION: enter");
		ret = nfc_ese_pwr(p61_dev->nfcc_data,
				  (arg == 1 ? ESE_RST_PROT_EN :
					      ESE_RST_PROT_DIS));
		P61_DBG_MSG(KERN_ALERT " PERFORM_RESET_PROTECTION ret: %d exit",
			    ret);
		break;
#ifdef CLOCK_SPI
	case ENBLE_SPI_CLK:
...
static int p61_parse_dt(struct device *dev, struct p61_spi_platform_data *data)
{
	...
	if (errorno < 0)
		P61_DBG_MSG("p61_parse_dt Failed to select default, errorno:%d",
			    errorno);
...
static const struct spi_device_id p61_id[] = { { "p61", 0 }, {} };
