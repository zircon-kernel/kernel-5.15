...
static void guidev_hw_reset(uint32_t mdelay)
{
	...
	usleep_range(GPIO_VEN_SET_WAIT_TIME_US,
		     (GPIO_VEN_SET_WAIT_TIME_US) + 100);
...
static int guidev_parse_dts(struct guide_dev *gdata)
{
	...
	gdata->hw_res.ven_gpio = of_get_named_gpio_flags(
		np, "tms,ven-gpio", 0, &gdata->hw_res.ven_flag);
	...
	gdata->hw_res.download_gpio = of_get_named_gpio_flags(
		np, "tms,download-gpio", 0, &gdata->hw_res.download_flag);

	if (gpio_is_valid(gdata->hw_res.download_gpio)) {
		rc = gpio_request(gdata->hw_res.download_gpio,
				  "guidev_fw_download");
	...
	TMS_DEBUG("irq_gpio = %d, ven_gpio = %d, dwnld_gpio = %d, error:%d\n",
		  gdata->hw_res.irq_gpio, gdata->hw_res.ven_gpio,
		  gdata->hw_res.download_gpio, ret);
	TMS_INFO("Normal end-\n");
	return SUCCESS;
err_fw_download:
	gpio_free(gdata->hw_res.ven_gpio);
...
static int guidev_gpio_configure(struct hw_resource hw_res)
{
	...
	if (gpio_is_valid(hw_res.download_gpio)) {
		ret = gpio_direction_output(hw_res.download_gpio,
					    hw_res.download_flag);
	...
	do {
		...
		if (gpio_get_value(gdata->hw_res.irq_gpio)) {
			...
			if (read_len > (MAX_CMD_LEN - info->check_sum)) {
				...
				TMS_WARN(
					"Receive payload is more than buf max\n");
			}
	...
	while (retry > 0) {
		...
		if (ret == info.write_len) {
			...
			if (info.pattern == TMS_FW) {
				...
				info.check_sum = 0;
				ret = tms_receive_process(gdata, &info,
							  read_buf);
			} else if (info.pattern == TMS_BL) {
...
static int guidev_chip_match(struct guide_dev *gdata)
{
	...
	struct match_info info_list[] = {
		...
		},
		{
			...
			.ver_num = 1,
			.major_ver = { 0xB0 },
			.cmd = { 0x5A, 0x00, 0x05, 0x00, 0x4a, 0x02, 0x00, 0x00,
				 0x17 },
		},
		{
			.type = NXP_SN110X,
...
static const struct i2c_device_id guide_dev_id[] = { { GUIDEDEV_NAME, 0 }, {} };
...
static const struct dev_pm_ops guide_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	guide_suspend, guide_resume) };
