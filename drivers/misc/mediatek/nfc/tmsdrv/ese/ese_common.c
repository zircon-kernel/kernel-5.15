...
static int ese_gpio_configure_init(struct ese_info *ese)
{
	...
	if (gpio_is_valid(ese->hw_res.rst_gpio)) {
		ret = gpio_direction_output(ese->hw_res.rst_gpio,
					    ese->hw_res.rst_flag);
...
static int ese_parse_dts_init(struct ese_info *ese)
{
	...
	ese->independent_support =
		of_property_read_bool(np, "independent_support");
	...
	if (ese->independent_support) {
		...
		ese->hw_res.rst_gpio = of_get_named_gpio_flags(
			np, "tms,reset-gpio", 0, &ese->hw_res.rst_flag);
		...
		TMS_DEBUG("irq_gpio = %d, rst_gpio = %d\n",
			  ese->hw_res.irq_gpio, ese->hw_res.rst_gpio);
	}
