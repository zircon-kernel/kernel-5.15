...
static int i2c_master_block_recv(struct nfc_info *nfc, uint8_t *buf,
			...
			loff_t *offset)
{
	...
	if (count > 0 && count < MAX_BUFFER_SIZE) {
		...
		TMS_WARN(
			"The read bytes[%zu] exceeded the buffer max size, count = %d\n",
				...
				size_t count, loff_t *offset)
{
	...
	if (count > 0 && count < MAX_BUFFER_SIZE) {
		...
		TMS_WARN(
			"The write bytes[%zu] exceeded the buffer max size, count = %d\n",
			count, MAX_BUFFER_SIZE);
		count = MAX_BUFFER_SIZE;
	} else {
	...
	if (write_buf[0] != T1_HEAD) {
		...
		if (elapse_time >= I2C_ELAPSE_TIMEOUT) {
			...
			while (++retry_count < MAX_I2C_WAKEUP_TIME) {
				ret = i2c_master_send(nfc->client, wakeup_cmd,
						      wakeup_len);
				usleep_range(I2C_WAKEUP_SLEEP_TIME1,
					     I2C_WAKEUP_SLEEP_TIME2);
				if (ret == wakeup_len) {
					break;
				}
			}
...
#if !defined(CONFIG_TMS_GUIDE_DEVICE) &&                                       \
	!defined(CONFIG_TMS_GUIDE_DEVICE_MODULE)
...
static const struct i2c_device_id nfc_device_id[] = { { NFC_DEVICE, 0 }, {} };
