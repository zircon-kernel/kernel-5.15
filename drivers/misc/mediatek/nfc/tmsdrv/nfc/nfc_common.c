...
ssize_t nfc_read(struct nfc_info *nfc, struct capsule *data, bool block_mode)
{
	...
	for (count = 0; count < RETRY_TIMES; count++) {
		...
		if (true == block_mode &&
		    (!gpio_get_value(nfc->hw_res.irq_gpio))) {
			TMS_DEBUG("Wait for data...\n");
			continue;
		} else if (false == block_mode &&
			   (!gpio_get_value(nfc->hw_res.irq_gpio))) {
		...
		ret = i2c_master_recv(nfc->client, &data->nci_recv[data->len],
				      read_len);
		if (ret == read_len) {
			     ...
			     char *buf)
{
	...
	for (count = 0; count < ARRAY_SIZE(check_sim); count++) {
		...
		for (rcv_count = 0; rcv_count < check_sim[count].count;
		     rcv_count++) {
	...
	for (count = 0; count < ARRAY_SIZE(chk_sim); count++) {
		if (count < recv.len &&
		    recv.nci_recv[count] == chk_sim[count]) {
			continue;
		} else {
			goto err_chk;
		}
...
static int nfc_gpio_configure_init(struct nfc_info *nfc)
{
	...
	if (gpio_is_valid(nfc->hw_res.ven_gpio)) {
		ret = gpio_direction_output(nfc->hw_res.ven_gpio,
					    nfc->hw_res.ven_flag);
...
static int nfc_parse_dts_init(struct nfc_info *nfc)
{
	...
	nfc->hw_res.download_gpio = of_get_named_gpio_flags(
		np, "tms,download-gpio", 0, &nfc->hw_res.download_flag);

	if (gpio_is_valid(nfc->hw_res.download_gpio)) {
		rcv = gpio_request(nfc->hw_res.download_gpio,
				   "nfc_fw_download");
