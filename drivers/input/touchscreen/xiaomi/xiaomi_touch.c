...
static int xiaomi_touch_dev_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (!(pdata->phy_base <= pos &&
	      ((pos + size) <= (pdata->phy_base + RAW_SIZE)))) {
		return -EOVERFLOW;
	}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	memcpy((unsigned char *)touch_pdata->raw_data,
	       (unsigned char *)touch_pdata->raw_buf[touch_pdata->raw_head],
	       touch_pdata->raw_len);
	spin_lock(&touch_pdata->raw_lock);
...
static DEVICE_ATTR(touch_thp_cmd_data, (0664), thp_cmd_data_show,
		   thp_cmd_data_store);

static DEVICE_ATTR(touch_thp_cmd, (0664), thp_cmd_status_show,
		   thp_cmd_status_store);

static DEVICE_ATTR(touch_thp_islandthd, (0664), thp_islandthreshold_show,
		   thp_islandthreshold_store);

static DEVICE_ATTR(touch_thp_downthd, (0664), thp_downthreshold_show,
		   thp_downthreshold_store);

static DEVICE_ATTR(touch_thp_upthd, (0664), thp_upthreshold_show,
		   thp_upthreshold_store);

static DEVICE_ATTR(touch_thp_movethd, (0664), thp_movethreshold_show,
		   thp_movethreshold_store);

static DEVICE_ATTR(touch_thp_smooth, (0664), thp_smooth_show, thp_smooth_store);

static DEVICE_ATTR(touch_thp_dump, (0664), thp_dump_frame_show,
		   thp_dump_frame_store);

static DEVICE_ATTR(touch_thp_noisefilter, (0664), thp_noisefilter_show,
		   thp_noisefilter_store);

static DEVICE_ATTR(enable_touch_raw, (0664), enable_touchraw_show,
		   enable_touchraw_store);

static DEVICE_ATTR(enable_touch_delta, (0664), enable_touchdelta_show,
		   enable_touchdelta_store);

static DEVICE_ATTR(palm_sensor, (0664), palm_sensor_show, palm_sensor_store);

static DEVICE_ATTR(prox_sensor, (0664), prox_sensor_show, prox_sensor_store);

static DEVICE_ATTR(clicktouch_raw, (0664), enable_clicktouch_show,
		   enable_clicktouch_store);
...
static DEVICE_ATTR(touch_thp_x_resolution, (0444),
		   xiaomi_touch_x_resolution_show, NULL);

static DEVICE_ATTR(touch_thp_y_resolution, (0444),
...
static int xiaomi_touch_probe(struct platform_device *pdev)
{
	...
	pdata->touch_data[0] =
		kzalloc(sizeof(struct xiaomi_touch_interface), GFP_KERNEL);
	...
	pdata->last_touch_events =
		kzalloc(sizeof(struct last_touch_event), GFP_KERNEL);
