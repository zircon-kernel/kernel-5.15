...
#define GF_NETLINK_ROUTE                                                       \
	29 /* for GF test temporary, need defined in include/uapi/linux/netlink.h */
...
static int power_is_enable = 0;
extern int mtk_drm_early_resume(int timeout);
struct spi_device *spi_fingerprint;
struct TEEC_UUID uuid_ta_gf = { 0x8888c03f,
				0xc30c,
				0x4dd0,
				{ 0xa3, 0x19, 0xea, 0x29, 0x64, 0x3d, 0x4d,
				  0x4b } };
...
static ssize_t gf_debug_show(struct device *dev, struct device_attribute *attr,
			     char *buf);
...
static struct attribute *gf_debug_attrs[] = { &dev_attr_debug.attr, NULL };

static struct attribute *performance_attrs[] = { &dev_attr_performance.attr,
						 NULL };
...
static void gf_power_enable_pins_reset(struct gf_device *gf_dev)
{
#ifdef CONFIG_OF
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	...
	if (onoff && !power_is_enable) {
		...
		if (buck == NULL) {
			gf_debug(INFO_LOG, "regulator_get zircon vfp fail");
			return;
		}

		if (regulator_is_enabled(buck)) {
			pr_err("%s: regulator_is_enabled!\n", __func__);
			gf_power_enable_pins_reset(gf_dev);
			return;
		}

		status = regulator_set_voltage(buck, 3000000, 3000000);
		if (status < 0) {
			gf_debug(INFO_LOG, "regulator_set fail,status = %d",
				 status);
			return;
		}

		rc = regulator_set_load(buck, 200000);
		if (rc) {
			pr_err("fp %s: set load faild\n", __func__);
			regulator_put(buck);
			return;
		}
		rc = regulator_enable(buck);
		if (rc) {
			pr_err("fp %s:enable voltage failed\n", __func__);
			return;
		}
		mdelay(15);
		gf_power_enable_pins_reset(gf_dev);
		...
		if (regulator_is_enabled(buck)) {
			...
			pr_err("regulator is already disabled \n");
			regulator_put(buck);
			return;
		}
		...
		regulator_put(buck);
		power_is_enable = 0;
		pr_err("disable_regulator finish\n");
		return;
	}
}
		       ...
		       loff_t *f_pos)
{
	...
	if (checksum != ((*(transfer_buf + count + 8) << 8) |
...
static ssize_t gf_write(struct file *filp, const char __user *buf, size_t count,
			loff_t *f_pos)
{
			      ...
			      const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-8", 2)) {
		...
		gf_debug(INFO_LOG,
			 "%s: parameter is -15, power reset test --> 0x\n",
			 __func__);
...
#ifdef SUPPORT_REE_SPI
#ifdef SUPPORT_REE_OSWEGO
static const char *oswego_m_sensor_type[] = { "GF316M", "GF318M",  "GF3118M",
					      "GF518M", "GF5118M", "GF516M",
					      "GF816M" };
	...
	*(tmp_buf + 4) = 0xF1;
	...
	if (retry) {
		...
		*(tmp_buf + 2) = (u8)(addr & 0xFF);
		...
		xfer[2].len = 3;
		xfer[2].delay.value = 5;
		xfer[2].delay.unit = SPI_DELAY_UNIT_USECS;
		...
		*(tmp_buf + 4) = 0xF1;
		...
		xfer[3].delay.value = 5;
		xfer[3].delay.unit = SPI_DELAY_UNIT_USECS;
...
static int gf_probe(struct spi_device *spi)
{
	...
	gf_debug(ERR_LOG, "%s, zircon try to get mt6368_vfp success\n",
		 __func__);

	gf_debug(ERR_LOG, "%s, try to set zircon\n", __func__);
	...
	if (IS_ERR(gf_dev->pins_spiio_spi_mode)) {
		ret = PTR_ERR(gf_dev->pins_spiio_spi_mode);
		gf_debug(ERR_LOG,
			 "%s fingerprint pinctrl spiio_spi_mode NULL\n",
			 __func__);
		return -1;
	} else {
		pinctrl_select_state(gf_dev->pinctrl_gpios,
	...
	if ((rx_test[0] != 0x12) || (rx_test[1] != 0x31)) {
		gf_debug(ERR_LOG, "%s, fail to get chip id 0x1231, zircon.\n",
			 __func__);
...
#ifdef SUPPORT_REE_OSWEGO
	{
		...
		sensor_num = sizeof(oswego_m_sensor_type) /
	...
	if (GF_DEV_MAJOR > 0) {
		...
		status = alloc_chrdev_region(
			&gf_dev->devno, gf_dev->device_count++, 1, GF_DEV_NAME);
	}
	if (status < 0) {
	...
	/* create device */
	gf_dev->device = device_create(gf_dev->class, &spi->dev, gf_dev->devno,
				       gf_dev, GF_DEV_NAME);
