 ...
 *
 ******************************************************/
static int awinic_i2c_read(struct awinic *awinic, unsigned char reg_addr,
...
static int awinic_i2c_write(struct awinic *awinic, unsigned char reg_addr,
			    unsigned char reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
	...
	do {
		if (aw86927->curr_buf->status == MMAP_BUF_DATA_VALID) {
			...
			if (samples_left < fifo_len) {
				memcpy(&aw86927->rtp_ptr[pos],
				       &aw86927->curr_buf->data[aw86927->pos],
				       samples_left);
				pos += samples_left;
				fifo_len -= samples_left;
				aw86927->curr_buf->status =
					MMAP_BUF_DATA_INVALID;
				aw86927->curr_buf->length = 0;
				aw86927->curr_buf =
					aw86927->curr_buf->kernel_next;
				aw86927->pos = 0;
			} else {
				memcpy(&aw86927->rtp_ptr[pos],
				       &aw86927->curr_buf->data[aw86927->pos],
		...
		} else if (aw86927->curr_buf->status ==
			   MMAP_BUF_DATA_FINISHED) {
			break;
		} else {
			if (retry-- <= 0) {
...
void richtap_rtp_work(struct work_struct *work)
{
	struct aw86927 *aw86927 =
		container_of(work, struct aw86927, richtap_rtp_work);
	...
	do {
		if (aw86927->curr_buf->status == MMAP_BUF_DATA_VALID) {
			if ((tmp_len + aw86927->curr_buf->length) >
			    aw86927->ram.base_addr) {
				memcpy(&aw86927->rtp_ptr[tmp_len],
				       aw86927->curr_buf->data,
				       (aw86927->ram.base_addr - tmp_len));
				aw86927->pos = aw86927->ram.base_addr - tmp_len;
				tmp_len = aw86927->ram.base_addr;
			} else {
				memcpy(&aw86927->rtp_ptr[tmp_len],
				       aw86927->curr_buf->data,
				       aw86927->curr_buf->length);
				 ...
				 unsigned long arg)
{
	...
	case RICHTAP_RTP_MODE:
		...
		aw86927_haptic_stop(aw86927);
		mutex_unlock(&aw86927->lock);
		if (copy_from_user(aw86927->rtp_ptr, (void __user *)arg,
		...
		if (richtap_rtp_start(aw86927)) {
			aw86927_write_rtp_data(aw86927, &aw86927->rtp_ptr[4],
					       tmp);
		}
		break;
...
ssize_t richtap_read(struct file *file, char __user *user_buf, size_t count,
		     loff_t *ppos)
{
	return 0;
}

ssize_t richtap_write(struct file *file, const char __user *user_buf,
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (awinic->name == AW8697) {
		awinic->aw8697 = devm_kzalloc(&i2c->dev, sizeof(struct aw8697),
					      GFP_KERNEL);
		...
		if (gpio_is_valid(awinic->aw8697->irq_gpio) &&
			...
			ret = devm_request_threaded_irq(
				&i2c->dev,
				gpio_to_irq(awinic->aw8697->irq_gpio), NULL,
				aw8697_irq, irq_flags, "aw8697",
				awinic->aw8697);
			if (ret != 0) {
...
#ifdef AAC_RICHTAP_SUPPORT
		awinic->aw86927->rtp_ptr =
			kmalloc(RICHTAP_MMAP_BUF_SIZE * RICHTAP_MMAP_BUF_SUM,
				GFP_KERNEL);
		...
		awinic->aw86927->start_buf =
			(struct mmap_buf_format *)__get_free_pages(
...
static int awinic_i2c_remove(struct i2c_client *i2c)
{
	...
	if (awinic->name == AW8697) {
		...
		free_pages((unsigned long)awinic->aw86927->start_buf,
