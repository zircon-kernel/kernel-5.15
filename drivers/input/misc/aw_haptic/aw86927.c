...
int aw86927_write_rtp_data(struct aw86927 *aw86927, unsigned char *val,
			   unsigned size)
{
	aw_dbg("%s enter!\n", __func__);
...
void aw86927_haptic_upload_lra(struct aw86927 *aw86927, unsigned int flag)
{
	unsigned char reg_val;
...
int aw86927_haptic_set_bst_vol(struct aw86927 *aw86927, unsigned char bst_vol)
{
	aw_dbg("%s enter!\n", __func__);
...
int aw86927_haptic_get_glb_state(struct aw86927 *aw86927, unsigned char *state)
{
	aw_dbg("%s enter!\n", __func__);
...
int aw86927_haptic_play_mode(struct aw86927 *aw86927, unsigned char play_mode)
{
	aw_dbg("%s enter!\n", __func__);
...
irqreturn_t aw86927_irq(int irq, void *data)
{
	...
	if (atomic_read(&aw86927->richtap_rtp_mode) == true) {
		if (aw86927_get_irq_state(aw86927)) {
			...
			richtap_update_fifo_data(
				aw86927, (aw86927->ram.base_addr -
					  (aw86927->ram.base_addr >> 2)));
			//richtap_update_fifo_data(aw_haptic, (aw_haptic->ram.base_addr >> 1));
			while (!aw86927_rtp_get_fifo_afi(aw86927) &&
			       atomic_read(&aw86927->richtap_rtp_mode) &&
			       (aw86927->curr_buf->status ==
				MMAP_BUF_DATA_VALID)) {
				richtap_update_fifo_data(
					aw86927, (aw86927->ram.base_addr >> 2));
			}
		}
