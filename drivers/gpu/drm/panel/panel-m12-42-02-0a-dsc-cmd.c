...
static void lcm_dcs_write(struct lcm *ctx, const void *data, size_t len)
{
	...
	if (is_mi_dev_support_iris() && iris_is_pt_mode()) {
		ret = 0;
		iris_panel_cmd_passthrough((int)*addr, len - 1, (u8 *)&data[1],
					   NULL, 0, 0);
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (ctx->gir_status) {
		push_table(ctx, gir_on_reload,
			   sizeof(gir_on_reload) /
				   sizeof(struct LCM_setting_table));

		if (ctx->dynamic_fps == 144) {
			if (panel_build_id == PANEL_APL2600 ||
			    panel_build_id == PANEL_APL2600_MP) {
				push_table(
					ctx, mode_144hz_setting_gir_on_peak2600,
					sizeof(mode_144hz_setting_gir_on_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_144hz_setting_gir_on,
					sizeof(mode_144hz_setting_gir_on) /
						sizeof(struct LCM_setting_table));
			}
			...
			if (panel_build_id == PANEL_APL2600 ||
			    panel_build_id == PANEL_APL2600_MP) {
				push_table(
					ctx, mode_120hz_setting_gir_on_peak2600,
					sizeof(mode_120hz_setting_gir_on_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_120hz_setting_gir_on,
			...
			if (panel_build_id == PANEL_APL2600 ||
			    panel_build_id == PANEL_APL2600_MP) {
				push_table(
					ctx, mode_90hz_setting_gir_on_peak2600,
					sizeof(mode_90hz_setting_gir_on_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_90hz_setting_gir_on,
			...
			if (panel_build_id == PANEL_APL2600 ||
			    panel_build_id == PANEL_APL2600_MP) {
				push_table(
					ctx, mode_60hz_setting_gir_on_peak2600,
					sizeof(mode_60hz_setting_gir_on_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_60hz_setting_gir_on,
		...
		if (ctx->dynamic_fps == 144) {
			if (panel_build_id == PANEL_APL2600 ||
				...
				push_table(
					ctx,
					mode_144hz_setting_gir_off_peak2600,
					sizeof(mode_144hz_setting_gir_off_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_144hz_setting_gir_off,
					sizeof(mode_144hz_setting_gir_off) /
						sizeof(struct LCM_setting_table));
			}
		} else if (ctx->dynamic_fps == 120) {
			if (panel_build_id == PANEL_APL2600 ||
				...
				push_table(
					ctx,
					mode_120hz_setting_gir_off_peak2600,
					sizeof(mode_120hz_setting_gir_off_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_120hz_setting_gir_off,
			...
			if (panel_build_id == PANEL_APL2600 ||
			    panel_build_id == PANEL_APL2600_MP) {
				push_table(
					ctx, mode_90hz_setting_gir_off_peak2600,
					sizeof(mode_90hz_setting_gir_off_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_90hz_setting_gir_off,
			...
			if (panel_build_id == PANEL_APL2600 ||
			    panel_build_id == PANEL_APL2600_MP) {
				push_table(
					ctx, mode_60hz_setting_gir_off_peak2600,
					sizeof(mode_60hz_setting_gir_off_peak2600) /
						sizeof(struct LCM_setting_table));
			} else {
				push_table(
					ctx, mode_60hz_setting_gir_off,
					sizeof(mode_60hz_setting_gir_off) /
						sizeof(struct LCM_setting_table));
			}
		}
		push_table(ctx, gir_off_gamma_update,
			   sizeof(gir_off_gamma_update) /
				   sizeof(struct LCM_setting_table));
	}

	push_table(ctx, sleep_out_setting,
		   sizeof(sleep_out_setting) /
			   sizeof(struct LCM_setting_table));
	if (panel_build_id == PANEL_APL2600 ||
	    panel_build_id == PANEL_APL2600_MP) {
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pr_info("%s\n", __func__);
	ctx->cam_gpio =
		devm_gpiod_get_index(ctx->dev, "cam", 0, GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->cam_gpio)) {
		dev_err(ctx->dev, "%s: cannot get cam gpio %ld\n", __func__,
			PTR_ERR(ctx->cam_gpio));
	}
...
#if defined(CONFIG_PXLW_IRIS)
static void iris_setbacklight(void *dsi, dcs_write_gce cb, void *handle,
			      char *src, int s_len)
{
	char dest[512] = {
		0,
...
static void lcm_set_hbm_demura(void *dsi, dcs_write_gce cb, void *handle,
			       bool enable)
{
...
static int lcm_handle_hbm_dbi_setting(void *dsi, dcs_write_gce cb, void *handle,
				      unsigned int level)
{
	static int last_hbm_dbi_status = -1;

	/* for dbv <= 0x7ff(2047) or dbv >= 0xE11(3601)*/
	if (level <= DBI_SETTING_1_THRESHOLD ||
	    level > DBI_SETTING_OFF_THRESHOLD) {
		dbi_setting_status = DBI_SETTING_OFF;
		/* for dbv >= 0x800(2048) and dbv <= 0x915(2325)*/
	} else if (level > DBI_SETTING_1_THRESHOLD &&
		   level <= DBI_SETTING_2_THRESHOLD) {
		dbi_setting_status = DBI_SETTING_1;
		/* for dbv >= 0x916(2326) and dbv <= 0xA8C(2700)*/
	} else if (level > DBI_SETTING_2_THRESHOLD &&
		   level <= DBI_SETTING_3_THRESHOLD) {
		dbi_setting_status = DBI_SETTING_2;
		/* for dbv >= 0xA8D(2701) and dbv <= 0xC4E(3150)*/
	} else if (level > DBI_SETTING_3_THRESHOLD &&
		   level <= DBI_SETTING_4_THRESHOLD) {
		dbi_setting_status = DBI_SETTING_3;
		/* for dbv >= 0xC4F(3151) and dbv <= 0xE10(3600)*/
	} else if (level > DBI_SETTING_4_THRESHOLD &&
		   level <= DBI_SETTING_OFF_THRESHOLD) {
		dbi_setting_status = DBI_SETTING_4;
	}

	if (dbi_setting_status != last_hbm_dbi_status) {
		pr_info("%s enter update, DBI setting = %d", __func__,
			dbi_setting_status);
		switch (dbi_setting_status) {
		case DBI_SETTING_OFF:
			cb(dsi, handle, CMD_PAGE8, ARRAY_SIZE(CMD_PAGE8));
			cb(dsi, handle, DBI_SETTING_OFF_R,
			   ARRAY_SIZE(DBI_SETTING_OFF_R));
			cb(dsi, handle, DBI_SETTING_OFF_G,
			   ARRAY_SIZE(DBI_SETTING_OFF_G));
			cb(dsi, handle, DBI_SETTING_OFF_B,
			   ARRAY_SIZE(DBI_SETTING_OFF_B));
			break;
		case DBI_SETTING_1:
			cb(dsi, handle, CMD_PAGE8, ARRAY_SIZE(CMD_PAGE8));
			cb(dsi, handle, DBI_SETTING_1_R,
			   ARRAY_SIZE(DBI_SETTING_1_R));
			cb(dsi, handle, DBI_SETTING_1_G,
			   ARRAY_SIZE(DBI_SETTING_1_G));
			cb(dsi, handle, DBI_SETTING_1_B,
			   ARRAY_SIZE(DBI_SETTING_1_B));
			break;
		case DBI_SETTING_2:
			cb(dsi, handle, CMD_PAGE8, ARRAY_SIZE(CMD_PAGE8));
			cb(dsi, handle, DBI_SETTING_2_R,
			   ARRAY_SIZE(DBI_SETTING_2_R));
			cb(dsi, handle, DBI_SETTING_2_G,
			   ARRAY_SIZE(DBI_SETTING_2_G));
			cb(dsi, handle, DBI_SETTING_2_B,
			   ARRAY_SIZE(DBI_SETTING_2_B));
			break;
		case DBI_SETTING_3:
			cb(dsi, handle, CMD_PAGE8, ARRAY_SIZE(CMD_PAGE8));
			cb(dsi, handle, DBI_SETTING_3_R,
			   ARRAY_SIZE(DBI_SETTING_3_R));
			cb(dsi, handle, DBI_SETTING_3_G,
			   ARRAY_SIZE(DBI_SETTING_3_G));
			cb(dsi, handle, DBI_SETTING_3_B,
			   ARRAY_SIZE(DBI_SETTING_3_B));
			break;
		case DBI_SETTING_4:
			cb(dsi, handle, CMD_PAGE8, ARRAY_SIZE(CMD_PAGE8));
			cb(dsi, handle, DBI_SETTING_4_R,
			   ARRAY_SIZE(DBI_SETTING_4_R));
			cb(dsi, handle, DBI_SETTING_4_G,
			   ARRAY_SIZE(DBI_SETTING_4_G));
			cb(dsi, handle, DBI_SETTING_4_B,
			   ARRAY_SIZE(DBI_SETTING_4_B));
			break;
		default:
				 ...
				 struct mtk_bl_ext_config *bl_ext_config)
{
	...
	if (bl_ext_config->cfg_flag & (0x1 << SET_BACKLIGHT_LEVEL)) {
		...
		if (atomic_read(&doze_enable)) {
			pr_info("%s: Return it when aod on, %d %d %d\n",
				__func__, level, bl_tb[0].para_list[1],
				bl_tb[0].para_list[2]);
			if (need_cfg_elvss) {
				pr_info("%s elvss = -%d\n", __func__, pulses);
				elvss_tb[0].para_list[1] =
					(u8)((1 << 7) | pulses);
//#if defined(CONFIG_PXLW_IRIS)
#if 0
...
static void panel_update_gir_setting()
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (panel_build_id == PANEL_APL2600 ||
		    panel_build_id == PANEL_APL2600_MP) {
			mode_144hz_setting_gir_off_peak2600[sGiroff_144hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_144[i];
			mode_144hz_setting_gir_on_peak2600[sGiron_144hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_144[i];

			mode_120hz_setting_gir_off_peak2600[sGiroff_120hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			mode_120hz_setting_gir_on_peak2600[sGiron_120hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];

			mode_90hz_setting_gir_off_peak2600[sGiroff_90hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			mode_90hz_setting_gir_on_peak2600[sGiron_90hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];

			mode_60hz_setting_gir_off_peak2600[sGiroff_60hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			mode_60hz_setting_gir_on_peak2600[sGiron_60hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];
			...
			pr_info("mode_switch peak gamma setting: 0x%02hhx 0x%02hhx 0x%02hhx 0x%02hhx 0x%02hhx 0x%02hhx 0x%02hhx 0x%02hhx\n",
				mode_144hz_setting_gir_off_peak2600
					[sGiroff_144hz_Index]
						.para_list[i],
				mode_144hz_setting_gir_on_peak2600
					[sGiron_144hz_Index]
						.para_list[i],
				mode_120hz_setting_gir_off_peak2600
					[sGiroff_120hz_Index]
						.para_list[i],
				mode_120hz_setting_gir_on_peak2600
					[sGiron_120hz_Index]
						.para_list[i],
				mode_90hz_setting_gir_off_peak2600
					[sGiroff_90hz_Index]
						.para_list[i],
				mode_90hz_setting_gir_on_peak2600
					[sGiron_90hz_Index]
						.para_list[i],
				mode_60hz_setting_gir_off_peak2600
					[sGiroff_60hz_Index]
						.para_list[i],
				mode_60hz_setting_gir_on_peak2600
					[sGiron_60hz_Index]
						.para_list[i]);

			// for gir switch
			gir_off_120hz_settings[sGir_Switch_off_120hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];
			gir_off_120hz_settings[sGir_Switch_off_120hz_Index + 1]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			gir_on_120hz_settings[sGir_Switch_on_120hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];
			gir_on_120hz_settings[sGir_Switch_on_120hz_Index + 1]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];

			gir_off_not_120hz_settings_peak2600
				[sGir_Switch_off_not_60hz_Index]
					.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			gir_on_not_120hz_settings_peak2600
				[sGir_Switch_on_not_60hz_Index]
					.para_list[i] =
				sReloadgamma_table.gir_off_120[i];

			gir_off_144hz_peak2600[sGiroff_144hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_144[i];
			gir_on_144hz_peak2600[sGiron_144hz_Index].para_list[i] =
				sReloadgamma_table.gir_on_144[i];

			gir_off_90hz_peak2600[sGiroff_90hz_Index].para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			gir_on_90hz_peak2600[sGiron_90hz_Index].para_list[i] =
				sReloadgamma_table.gir_on_120[i];

			gir_off_60hz_peak2600[sGiroff_60hz_Index].para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			gir_on_60hz_peak2600[sGiron_60hz_Index].para_list[i] =
				sReloadgamma_table.gir_on_120[i];
		} else {
			mode_144hz_setting_gir_off[sGiroff_144hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_144[i];
			mode_144hz_setting_gir_on[sGiron_144hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_144[i];

			mode_120hz_setting_gir_off[sGiroff_120hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			mode_120hz_setting_gir_on[sGiron_120hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];

			mode_90hz_setting_gir_off[sGiroff_90hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			mode_90hz_setting_gir_on[sGiron_90hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];

			mode_60hz_setting_gir_off[sGiroff_60hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			mode_60hz_setting_gir_on[sGiron_60hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_on_120[i];
			...
			gir_off_not_120hz_settings[sGir_Switch_off_not_60hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			gir_on_not_120hz_settings[sGir_Switch_on_not_60hz_Index]
				.para_list[i] =
				sReloadgamma_table.gir_off_120[i];

			gir_off_144hz[sGiroff_144hz_Index].para_list[i] =
				sReloadgamma_table.gir_off_144[i];
			gir_on_144hz[sGiron_144hz_Index].para_list[i] =
				sReloadgamma_table.gir_on_144[i];

			gir_off_90hz[sGiroff_90hz_Index].para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			gir_on_90hz[sGiron_90hz_Index].para_list[i] =
				sReloadgamma_table.gir_on_120[i];

			gir_off_60hz[sGiroff_60hz_Index].para_list[i] =
				sReloadgamma_table.gir_off_120[i];
			gir_on_60hz[sGiron_60hz_Index].para_list[i] =
				sReloadgamma_table.gir_on_120[i];
		}
	}
	pr_info("%s -\n", __func__);
...
static void
panel_parse_reloadgamma_info(struct reloadgamma_table *reloadgamma_table)
{
	int i;
	...
	pr_info("%s: reload_gamma_info_cmdline:%s\n", __func__,
		reload_gamma_info_cmdline);
	for (i = 0; i < 16; i++) {
		sscanf(reload_gamma_info_cmdline + 2 * i, "%02hhx",
		       &reload_gamma_info[i]);
	}
...
#if defined(CONFIG_PXLW_IRIS)
	if (is_mi_dev_support_iris()) {
		iris_setbacklight(dsi, cb, handle, page0_tb,
				  ARRAY_SIZE(page0_tb));
		iris_setbacklight(dsi, cb, handle, data_remap_enable_tb,
				  ARRAY_SIZE(data_remap_enable_tb));
		iris_setbacklight(dsi, cb, handle, cmd0_tb,
				  ARRAY_SIZE(cmd0_tb));
		iris_setbacklight(dsi, cb, handle, cmd1_tb,
				  ARRAY_SIZE(cmd1_tb));
		iris_setbacklight(dsi, cb, handle, page_51,
				  ARRAY_SIZE(page_51));
		//usleep_range(50 * 1000, 50 * 1000 + 10);
		iris_setbacklight(dsi, cb, handle, cmd2_tb,
				  ARRAY_SIZE(cmd2_tb));
	} else {
		cb(dsi, handle, page0_tb, ARRAY_SIZE(page0_tb));
		cb(dsi, handle, data_remap_enable_tb,
		   ARRAY_SIZE(data_remap_enable_tb));
...
#if defined(CONFIG_PXLW_IRIS)
	if (is_mi_dev_support_iris()) {
		iris_setbacklight(dsi, cb, handle, gir_off_tb,
				  ARRAY_SIZE(gir_off_tb));
		iris_setbacklight(dsi, cb, handle, normal2aod_tb,
				  ARRAY_SIZE(normal2aod_tb));
		iris_setbacklight(dsi, cb, handle, page0_tb,
				  ARRAY_SIZE(page0_tb));
		iris_setbacklight(dsi, cb, handle, data_remap_diable_tb,
				  ARRAY_SIZE(data_remap_diable_tb));
		iris_setbacklight(dsi, cb, handle, cmd0_tb,
				  ARRAY_SIZE(cmd0_tb));
		usleep_range(20 * 1000, 20 * 1000 + 10);
		iris_setbacklight(dsi, cb, handle, cmd1_tb1,
				  ARRAY_SIZE(cmd1_tb1));
		iris_setbacklight(dsi, cb, handle, cmd2_tb2,
				  ARRAY_SIZE(cmd2_tb2));
	} else {
		cb(dsi, handle, gir_off_tb, ARRAY_SIZE(gir_off_tb));
		cb(dsi, handle, normal2aod_tb, ARRAY_SIZE(normal2aod_tb));
		cb(dsi, handle, page0_tb, ARRAY_SIZE(page0_tb));
		cb(dsi, handle, data_remap_diable_tb,
		   ARRAY_SIZE(data_remap_diable_tb));
		cb(dsi, handle, cmd0_tb, ARRAY_SIZE(cmd0_tb));
		usleep_range(20 * 1000, 20 * 1000 + 10);
				     ...
				     int doze_brightness)
{
	...
	if (DOZE_TO_NORMAL == doze_brightness) {
		ret = mi_disp_panel_ddic_send_cmd(doze_disable_t,
						  ARRAY_SIZE(doze_disable_t),
						  false, false);
		usleep_range(50 * 1000, 50 * 1000 + 10);
		if (ret == -1)
			pr_info("%s error\n", __func__);
		ret = mi_disp_panel_ddic_send_cmd(
			table_2C, ARRAY_SIZE(table_2C), false, false);
		atomic_set(&doze_enable, 0);
		ctx->doze_suspend = false;
		goto exit;
	} else if (DOZE_BRIGHTNESS_LBM == doze_brightness) {
		ret = mi_disp_panel_ddic_send_cmd(
			doze_enable_l, ARRAY_SIZE(doze_enable_l), false, false);

	} else if (DOZE_BRIGHTNESS_HBM == doze_brightness) {
		ret = mi_disp_panel_ddic_send_cmd(
			doze_enable_h, ARRAY_SIZE(doze_enable_h), false, false);
	}
	usleep_range(20 * 1000, 20 * 1000 + 10);
	ret = mi_disp_panel_ddic_send_cmd(table_652C, ARRAY_SIZE(table_652C),
					  false, false);
#else
	if (DOZE_TO_NORMAL == doze_brightness) {
		ret = mi_disp_panel_ddic_send_cmd(
			backlight_0, ARRAY_SIZE(backlight_0), false, false);
		atomic_set(&doze_enable, 0);
	} else if (DOZE_BRIGHTNESS_LBM == doze_brightness) {
		ret = mi_disp_panel_ddic_send_cmd(
			backlight_l, ARRAY_SIZE(backlight_l), false, false);

	} else if (DOZE_BRIGHTNESS_HBM == doze_brightness) {
		ret = mi_disp_panel_ddic_send_cmd(
			backlight_h, ARRAY_SIZE(backlight_h), false, false);
	}
...
static int panel_set_gray_by_temperature(struct drm_panel *panel, int level)
{
	...
	if (level == TEMP_INDEX1_35) {
		gray_3d_lut_on[3].para_list[0] = 0x02;
		mi_disp_panel_ddic_send_cmd(gray_3d_lut_on,
					    ARRAY_SIZE(gray_3d_lut_on), false,
					    false);
		...
		mi_disp_panel_ddic_send_cmd(gray_3d_lut_off,
					    ARRAY_SIZE(gray_3d_lut_off), false,
					    false);
	}
	...
	if (panel_build_id == PANEL_APL2600 ||
		...
		if (ctx->dynamic_fps == 120) {
			...
			mi_disp_panel_ddic_send_cmd(
				gir_on_120hz_settings,
				ARRAY_SIZE(gir_on_120hz_settings), true, false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			// reload gir on + 120hz gir on setting
			mi_disp_panel_ddic_send_cmd(
				gir_on_120hz_reload,
				ARRAY_SIZE(gir_on_120hz_reload), false, false);
		} else if (ctx->dynamic_fps == 60) {
			// demura reload + 120hz gir off setting
			mi_disp_panel_ddic_send_cmd(
				gir_on_not_120hz_settings_peak2600,
				ARRAY_SIZE(gir_on_not_120hz_settings_peak2600),
				true, false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			mi_disp_panel_ddic_send_cmd(
				fcon_reload_off2, ARRAY_SIZE(fcon_reload_off2),
				false, false);
			// 60hz gir on setting
			mi_disp_panel_ddic_send_cmd(
				gir_on_60hz_peak2600,
				ARRAY_SIZE(gir_on_60hz_peak2600), false, false);
		} else if (ctx->dynamic_fps == 90) {
			// demura reload + 120hz gir off setting
			mi_disp_panel_ddic_send_cmd(
				gir_on_not_120hz_settings_peak2600,
			...
			// 90hz gir on setting
			mi_disp_panel_ddic_send_cmd(
				gir_on_90hz_peak2600,
				ARRAY_SIZE(gir_on_90hz_peak2600), false, false);
			...
			mi_disp_panel_ddic_send_cmd(
				gir_on_144hz_peak2600,
				ARRAY_SIZE(gir_on_144hz_peak2600), false,
				false);
		}
	} else {
		if (ctx->dynamic_fps == 120) {
			...
			mi_disp_panel_ddic_send_cmd(
				gir_on_120hz_settings,
				ARRAY_SIZE(gir_on_120hz_settings), false,
				false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			// reload gir on + 120hz gir on setting
			mi_disp_panel_ddic_send_cmd(
			...
			// demura reload + 120hz gir off setting
			mi_disp_panel_ddic_send_cmd(
				gir_on_not_120hz_settings,
				ARRAY_SIZE(gir_on_not_120hz_settings), false,
				false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			mi_disp_panel_ddic_send_cmd(
				fcon_reload_off2, ARRAY_SIZE(fcon_reload_off2),
				false, false);
			// 60hz gir on setting
			mi_disp_panel_ddic_send_cmd(gir_on_60hz,
						    ARRAY_SIZE(gir_on_60hz),
						    false, false);
		} else if (ctx->dynamic_fps == 90) {
			// demura reload + 120hz gir off setting
			mi_disp_panel_ddic_send_cmd(
			...
			mi_disp_panel_ddic_send_cmd(
				...
				false, false);
			// 90hz gir on setting
			mi_disp_panel_ddic_send_cmd(gir_on_90hz,
						    ARRAY_SIZE(gir_on_90hz),
						    false, false);
		} else if (ctx->dynamic_fps == 144) {
			// demura reload + 120hz gir off setting
			mi_disp_panel_ddic_send_cmd(
			...
			mi_disp_panel_ddic_send_cmd(
				...
				false, false);
			// 144hz gir on setting
			mi_disp_panel_ddic_send_cmd(gir_on_144hz,
						    ARRAY_SIZE(gir_on_144hz),
						    false, false);
		}
	}
...
static int panel_set_gir_off(struct drm_panel *panel)
{
	...
	if (panel_build_id == PANEL_APL2600 ||
		...
		if (ctx->dynamic_fps == 120) {
			...
			mi_disp_panel_ddic_send_cmd(
				gir_off_120hz_settings,
				ARRAY_SIZE(gir_off_120hz_settings), true,
				false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			// reload gir off + 120hz gir off setting
			mi_disp_panel_ddic_send_cmd(
				gir_off_120hz_reload,
				ARRAY_SIZE(gir_off_120hz_reload), false, false);
		} else if (ctx->dynamic_fps == 60) {
			// demura reload + 120hz setting
			mi_disp_panel_ddic_send_cmd(
				gir_off_not_120hz_settings_peak2600,
				ARRAY_SIZE(gir_off_not_120hz_settings_peak2600),
				true, false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			// auto update gamma
			mi_disp_panel_ddic_send_cmd(fcon_reload_off,
						    ARRAY_SIZE(fcon_reload_off),
						    false, false);
			// 60hz gir off setting
			mi_disp_panel_ddic_send_cmd(
				gir_off_60hz_peak2600,
				ARRAY_SIZE(gir_off_60hz_peak2600), false,
				false);
		} else if (ctx->dynamic_fps == 90) {
			// demura reload + 120hz setting
			mi_disp_panel_ddic_send_cmd(
			...
			mi_disp_panel_ddic_send_cmd(
				gir_off_90hz_peak2600,
				ARRAY_SIZE(gir_off_90hz_peak2600), false,
				false);
		} else if (ctx->dynamic_fps == 144) {
			// demura reload + 120hz setting
			mi_disp_panel_ddic_send_cmd(
			...
			mi_disp_panel_ddic_send_cmd(
				gir_off_144hz_peak2600,
				ARRAY_SIZE(gir_off_144hz_peak2600), false,
				false);
		}
	} else {
		if (ctx->dynamic_fps == 120) {
			...
			mi_disp_panel_ddic_send_cmd(
				gir_off_120hz_settings,
				ARRAY_SIZE(gir_off_120hz_settings), false,
				false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			// reload gir off + 120hz gir off setting
			mi_disp_panel_ddic_send_cmd(
			...
			// demura reload + 120hz setting
			mi_disp_panel_ddic_send_cmd(
				gir_off_not_120hz_settings,
				ARRAY_SIZE(gir_off_not_120hz_settings), false,
				false);
			usleep_range(20 * 1000, 20 * 1000 + 10);
			// auto update gamma
			mi_disp_panel_ddic_send_cmd(fcon_reload_off,
						    ...
						    false, false);
			// 60hz gir off setting
			mi_disp_panel_ddic_send_cmd(gir_off_60hz,
						    ARRAY_SIZE(gir_off_60hz),
						    false, false);
		} else if (ctx->dynamic_fps == 90) {
			// demura reload + 120hz setting
			mi_disp_panel_ddic_send_cmd(
			...
			mi_disp_panel_ddic_send_cmd(fcon_reload_off,
						    ...
						    false, false);
			// 90hz gir off setting
			mi_disp_panel_ddic_send_cmd(gir_off_90hz,
						    ARRAY_SIZE(gir_off_90hz),
						    false, false);
		} else if (ctx->dynamic_fps == 144) {
			// demura reload + 120hz setting
			mi_disp_panel_ddic_send_cmd(
			...
			mi_disp_panel_ddic_send_cmd(fcon_reload_off,
						    ...
						    false, false);
			// 144hz gir off setting
			mi_disp_panel_ddic_send_cmd(gir_off_144hz,
						    ARRAY_SIZE(gir_off_144hz),
						    false, false);
		}
	}
...
static void mi_parse_cmdline_perBL(struct LHBM_WHITEBUF *lhbm_whitebuf)
{
	...
	if (!lhbm_w900_update_flag && !lhbm_w110_update_flag &&
	...
	for (i = 0; i < 6; i += 2) {
		if (panel_build_id == PANEL_APL2600 ||
		    panel_build_id == PANEL_APL2600_MP) {
			temp = ((int)(lhbm_cmdbuf[i + 18] * 256 +
				      lhbm_cmdbuf[i + 19]) *
				gamma_coffee_w900_gir_on[i / 2]) /
			       1000;
		} else {
			temp = (int)(lhbm_cmdbuf[i + 18] * 256 +
				     lhbm_cmdbuf[i + 19]);
	...
	for (i = 0; i < 2; i++) {
		if (panel_build_id == PANEL_APL2600 ||
			...
			lhbm_whitebuf->gir_off_500[i] =
				lhbm_cmdbuf[i + 24] * 1035 / 1000;
			lhbm_whitebuf->gir_on_500[i] =
				lhbm_cmdbuf[i + 26] * 1035 / 1000;
		}
	}
...
static int mi_disp_panel_update_lhbm_white_param(struct mtk_dsi *dsi,
						 ...
						 int flat_mode)
{
	...
	*/
	...
	if (!lhbm_w900_readbackdone || !lhbm_w110_readbackdone ||
	...
	case TYPE_WHITE_1200:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_normal_white_1200nit[8].para_list[i] =
					lhbm_whitebuf.gir_on_900[i];
			}
		} else {
			for (i = 0; i < 6; i++) {
				lhbm_normal_white_1200nit[8].para_list[i] =
					lhbm_whitebuf.gir_off_900[i];
			}
		}
		break;
	case TYPE_WHITE_250:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_normal_white_250nit[8].para_list[i] =
					lhbm_whitebuf.gir_on_110[i];
			}
		} else {
			for (i = 0; i < 6; i++) {
				lhbm_normal_white_250nit[8].para_list[i] =
					lhbm_whitebuf.gir_off_110[i];
			}
		}
		break;
	case TYPE_GREEN_500:
		if (flat_mode) {
			for (i = 0; i < 2; i++) {
				lhbm_normal_green_500nit[8].para_list[i + 2] =
					lhbm_whitebuf.gir_on_500[i];
			}
		} else {
			for (i = 0; i < 2; i++) {
	...
	case TYPE_HLPM_W1200:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_hlpm_white_1200nit[14].para_list[i] =
					lhbm_whitebuf.gir_on_900[i];
			}
		} else {
			for (i = 0; i < 6; i++) {
	...
	case TYPE_HLPM_W250:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_hlpm_white_250nit[14].para_list[i] =
					lhbm_whitebuf.gir_on_110[i];
			}
		} else {
			for (i = 0; i < 6; i++) {
...
static void mi_disp_panel_update_lhbm_51reg(struct mtk_dsi *dsi,
					    enum lhbm_cmd_type type,
					    int bl_level)
{
	u8 alpha_buf[2] = { 0 };
...
static void mi_disp_panel_update_lhbm_87reg(struct mtk_dsi *dsi,
					    enum lhbm_cmd_type type,
					    int bl_level)
{
	u8 alpha_buf[2] = { 0 };
			      ...
			      enum local_hbm_state lhbm_state)
{
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT:
		...
		if (bl_level)
			mi_disp_panel_update_lhbm_51reg(dsi, TYPE_LHBM_OFF,
							bl_level);
		else
			mi_disp_panel_update_lhbm_51reg(dsi, TYPE_LHBM_OFF,
							bl_level_doze);
	...
	case LOCAL_HBM_NORMAL_WHITE_110NIT:
		...
		if (atomic_read(&doze_enable)) {
			mi_disp_panel_update_lhbm_51reg(dsi, TYPE_HLPM_W250,
							bl_level_doze);
			mi_disp_panel_update_lhbm_87reg(dsi, TYPE_HLPM_W250,
							bl_level_doze);
			mi_disp_panel_update_lhbm_white_param(
				dsi, TYPE_HLPM_W250, flat_mode);
			mi_disp_panel_ddic_send_cmd(
				lhbm_hlpm_white_250nit,
				ARRAY_SIZE(lhbm_hlpm_white_250nit), false,
				true);
			ctx->doze_suspend = false;
		} else {
			mi_disp_panel_update_lhbm_51reg(dsi, TYPE_WHITE_250,
							bl_level);
			mi_disp_panel_update_lhbm_87reg(dsi, TYPE_WHITE_250,
							bl_level);
			mi_disp_panel_update_lhbm_white_param(
				dsi, TYPE_WHITE_250, flat_mode);
			mi_disp_panel_ddic_send_cmd(
				lhbm_normal_white_250nit,
				ARRAY_SIZE(lhbm_normal_white_250nit), false,
				true);
		}
		ctx->lhbm_en = true;
		break;
	case LOCAL_HBM_NORMAL_GREEN_500NIT:
		...
		mi_disp_panel_update_lhbm_white_param(dsi, TYPE_GREEN_500,
						      flat_mode);
		mi_disp_panel_ddic_send_cmd(
			lhbm_normal_green_500nit,
			ARRAY_SIZE(lhbm_normal_green_500nit), false, true);
		ctx->doze_suspend = false;
		ctx->lhbm_en = true;
		break;
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		if (atomic_read(&doze_enable)) {
			pr_info("for aod \n");
			mi_disp_panel_update_lhbm_51reg(dsi, TYPE_HLPM_W1200,
							bl_level_doze);
			mi_disp_panel_update_lhbm_87reg(dsi, TYPE_HLPM_W1200,
							bl_level_doze);
			mi_disp_panel_update_lhbm_white_param(
				dsi, TYPE_HLPM_W1200, flat_mode);
			mi_disp_panel_ddic_send_cmd(
				lhbm_hlpm_white_1200nit,
				ARRAY_SIZE(lhbm_hlpm_white_1200nit), false,
				true);
			ctx->doze_suspend = false;
		} else {
			pr_info("LOCAL_HBM_NORMAL_WHITE_1200NIT in HBM\n");
			mi_disp_panel_update_lhbm_51reg(dsi, TYPE_WHITE_1200,
							bl_level);
			mi_disp_panel_update_lhbm_87reg(dsi, TYPE_WHITE_1200,
							bl_level);
			mi_disp_panel_update_lhbm_white_param(
				dsi, TYPE_WHITE_1200, flat_mode);
			mi_disp_panel_ddic_send_cmd(
				lhbm_normal_white_1200nit,
				ARRAY_SIZE(lhbm_normal_white_1200nit), false,
				true);
		}
		ctx->lhbm_en = true;
		break;
	case LOCAL_HBM_HLPM_WHITE_110NIT:
		...
		mi_disp_panel_update_lhbm_white_param(dsi, TYPE_HLPM_W250,
						      flat_mode);
		mi_disp_panel_ddic_send_cmd(lhbm_hlpm_white_250nit,
					    ARRAY_SIZE(lhbm_hlpm_white_250nit),
					    false, true);
		ctx->doze_suspend = false;
		ctx->lhbm_en = true;
		break;
	case LOCAL_HBM_HLPM_WHITE_1000NIT:
		...
		mi_disp_panel_update_lhbm_white_param(dsi, TYPE_HLPM_W1200,
						      flat_mode);
		mi_disp_panel_ddic_send_cmd(lhbm_hlpm_white_1200nit,
					    ARRAY_SIZE(lhbm_hlpm_white_1200nit),
					    false, true);
		ctx->doze_suspend = false;
		ctx->lhbm_en = true;
		break;
...
static int panel_set_peak_hdr_status(struct mtk_dsi *dsi, int status)
{
	...
	if (status) {
		...
		mi_disp_panel_ddic_send_cmd(
			peak_hdr_on, ARRAY_SIZE(peak_hdr_on), false, false);
	} else {
		mi_disp_panel_ddic_send_cmd(
			peak_hdr_off, ARRAY_SIZE(peak_hdr_off), false, false);
		panel_set_gir_on(dsi->panel);
	}

	ctx->peak_hdr_status = status;
	pr_info("%s: peak_hdr_status = %d ctx->gir_status = %d\n", __func__,
		status, ctx->gir_status);
out:
	return 0;
err:
	return 1;
			 ...
			 struct drm_connector *connector)
{
	...
	if (!mode3) {
		dev_err(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			performence_mode_144.hdisplay,
			performence_mode_144.vdisplay,
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	ext_params.err_flag_irq_gpio = of_get_named_gpio_flags(
		...
		(enum of_gpio_flags *)&(ext_params.err_flag_irq_flags));
...
module_param_string(reload_gamma_info, reload_gamma_info_cmdline,
		    sizeof(reload_gamma_info_cmdline), 0600);
MODULE_PARM_DESC(reload_gamma_info,
		 "reload_gamma_info=<reload_gamma_table_info>");
