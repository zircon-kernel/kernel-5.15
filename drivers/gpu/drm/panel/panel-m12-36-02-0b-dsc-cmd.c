...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (panel_build_id == PANEL_APL2600) {
		...
		push_table(ctx, init_setting_peak2600_gamma,
			   sizeof(init_setting_peak2600_gamma) /
				   sizeof(struct LCM_setting_table));
		push_table(ctx, init_setting_peak2600,
			   sizeof(init_setting_peak2600) /
				   sizeof(struct LCM_setting_table));
	...
	if (ctx->gir_status) {
		...
		push_table(ctx, gir_on_not_144hz_for_panel_init,
			   sizeof(gir_on_not_144hz_for_panel_init) /
				   sizeof(struct LCM_setting_table));
	}
			       ...
			       bool enable)
{
	...
	char cmd0_hbm_demura_non_144hz[] = {
		0xF0, 0x55, 0xAA, 0x52, 0x08, 0x00
	};
	char cmd1_hbm_demura_non_144hz[] = { 0x6F, 0x2E };
	char cmd2_hbm_demura_non_144hz[] = {
		0xC0, 0x44, 0x44, 0x00, 0x00, 0x00
	};
	...
	char cmd0_lbm_demura_non_144hz[] = {
		0xF0, 0x55, 0xAA, 0x52, 0x08, 0x00
	};
	char cmd1_lbm_demura_non_144hz[] = { 0x6F, 0x2E };
	char cmd2_lbm_demura_non_144hz[] = {
		0xC0, 0x12, 0x34, 0x00, 0x00, 0x00
	};
	...
	if (enable == true) {
		if (ctx->dynamic_fps == 144) {
			cb(dsi, handle, cmd0_hbm_demura_144hz,
			   ARRAY_SIZE(cmd0_hbm_demura_144hz));
			cb(dsi, handle, cmd1_hbm_demura_144hz,
			   ARRAY_SIZE(cmd1_hbm_demura_144hz));
			cb(dsi, handle, cmd2_hbm_demura_144hz,
			   ARRAY_SIZE(cmd2_hbm_demura_144hz));
			cb(dsi, handle, cmd3_hbm_demura_144hz,
			   ARRAY_SIZE(cmd3_hbm_demura_144hz));
		} else {
			cb(dsi, handle, cmd0_hbm_demura_non_144hz,
			   ARRAY_SIZE(cmd0_hbm_demura_non_144hz));
			cb(dsi, handle, cmd1_hbm_demura_non_144hz,
			   ARRAY_SIZE(cmd1_hbm_demura_non_144hz));
			cb(dsi, handle, cmd2_hbm_demura_non_144hz,
			   ARRAY_SIZE(cmd2_hbm_demura_non_144hz));
		}
	} else {
		if (ctx->dynamic_fps == 144) {
			cb(dsi, handle, cmd0_lbm_demura_144hz,
			   ARRAY_SIZE(cmd0_lbm_demura_144hz));
			cb(dsi, handle, cmd1_lbm_demura_144hz,
			   ARRAY_SIZE(cmd1_lbm_demura_144hz));
			cb(dsi, handle, cmd2_lbm_demura_144hz,
			   ARRAY_SIZE(cmd2_lbm_demura_144hz));
			cb(dsi, handle, cmd3_lbm_demura_144hz,
			   ARRAY_SIZE(cmd3_lbm_demura_144hz));
		} else {
			cb(dsi, handle, cmd0_lbm_demura_non_144hz,
			   ARRAY_SIZE(cmd0_lbm_demura_non_144hz));
			cb(dsi, handle, cmd1_lbm_demura_non_144hz,
			   ARRAY_SIZE(cmd1_lbm_demura_non_144hz));
			cb(dsi, handle, cmd2_lbm_demura_non_144hz,
			   ARRAY_SIZE(cmd2_lbm_demura_non_144hz));
		}
	}
	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		if (panel_build_id == PANEL_APL2600) {
			push_table(ctx, mode_144hz_setting_gir_peak2600,
				   sizeof(mode_144hz_setting_gir_peak2600) /
					   sizeof(struct LCM_setting_table));
		} else if (panel_build_id == PANEL_APL2600_GAMMA) {
			push_table(
				ctx, mode_144hz_setting_gir_peak2600_gamma,
				sizeof(mode_144hz_setting_gir_peak2600_gamma) /
					sizeof(struct LCM_setting_table));
		}
		ctx->dynamic_fps = 144;
	}
	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		if (panel_build_id == PANEL_APL2600) {
			push_table(ctx, mode_non_144hz_setting_gir_peak2600,
				   sizeof(mode_non_144hz_setting_gir_peak2600) /
					   sizeof(struct LCM_setting_table));
		} else if (panel_build_id == PANEL_APL2600_GAMMA) {
			push_table(
				ctx, mode_non_144hz_setting_gir_peak2600_gamma,
				sizeof(mode_non_144hz_setting_gir_peak2600_gamma) /
					sizeof(struct LCM_setting_table));
		}
...
#if defined(CONFIG_PXLW_IRIS)
	if (is_mi_dev_support_iris()) {
		iris_setbacklight(dsi, cb, handle, page_f0,
				  ARRAY_SIZE(page_f0));
		iris_setbacklight(dsi, cb, handle, cmd0_disable_dic_el,
				  ARRAY_SIZE(cmd0_disable_dic_el));
		iris_setbacklight(dsi, cb, handle, cmd1_disable_dic_el,
				  ARRAY_SIZE(cmd1_disable_dic_el));
		iris_setbacklight(dsi, cb, handle, cmd2_disable_dic_el,
				  ARRAY_SIZE(cmd2_disable_dic_el));
		...
		iris_setbacklight(dsi, cb, handle, cmd2_tb,
				  ARRAY_SIZE(cmd2_tb));
		iris_setbacklight(dsi, cb, handle, esd_on_page1,
				  ARRAY_SIZE(esd_on_page1));
		iris_setbacklight(dsi, cb, handle, esd_on_page2,
				  ARRAY_SIZE(esd_on_page2));
		iris_setbacklight(dsi, cb, handle, esd_on_page3,
				  ARRAY_SIZE(esd_on_page3));
	} else {
		cb(dsi, handle, page_f0, ARRAY_SIZE(page_f0));
		cb(dsi, handle, cmd0_disable_dic_el,
		   ARRAY_SIZE(cmd0_disable_dic_el));
		cb(dsi, handle, cmd1_disable_dic_el,
		   ARRAY_SIZE(cmd1_disable_dic_el));
		cb(dsi, handle, cmd2_disable_dic_el,
		   ARRAY_SIZE(cmd2_disable_dic_el));
...
#if defined(CONFIG_PXLW_IRIS)
	if (is_mi_dev_support_iris()) {
		iris_setbacklight(dsi, cb, handle, esd_off_page1,
				  ARRAY_SIZE(esd_off_page1));
		iris_setbacklight(dsi, cb, handle, esd_off_page2,
				  ARRAY_SIZE(esd_off_page2));
		iris_setbacklight(dsi, cb, handle, esd_off_page3,
				  ARRAY_SIZE(esd_off_page3));
		iris_setbacklight(dsi, cb, handle, page_f0,
				  ARRAY_SIZE(page_f0));
		iris_setbacklight(dsi, cb, handle, cmd0_enable_dic_el,
				  ARRAY_SIZE(cmd0_enable_dic_el));
		iris_setbacklight(dsi, cb, handle, cmd1_enable_dic_el,
				  ARRAY_SIZE(cmd1_enable_dic_el));
		iris_setbacklight(dsi, cb, handle, cmd2_enable_dic_el,
				  ARRAY_SIZE(cmd2_enable_dic_el));
		iris_setbacklight(dsi, cb, handle, brightnessh_tb,
				  ARRAY_SIZE(brightnessh_tb));
		...
		cb(dsi, handle, page_f0, ARRAY_SIZE(page_f0));
		cb(dsi, handle, cmd0_enable_dic_el,
		   ARRAY_SIZE(cmd0_enable_dic_el));
		cb(dsi, handle, cmd1_enable_dic_el,
		   ARRAY_SIZE(cmd1_enable_dic_el));
		cb(dsi, handle, cmd2_enable_dic_el,
		   ARRAY_SIZE(cmd2_enable_dic_el));
...
static int panel_set_gir_on(struct drm_panel *panel)
{
	...
	if (ctx->dynamic_fps == 144) {
		mi_disp_panel_ddic_send_cmd(gir_on_144hz_settings,
					    ARRAY_SIZE(gir_on_144hz_settings),
					    false, false);
	} else {
		mi_disp_panel_ddic_send_cmd(
			gir_on_not_144hz_settings,
...
static int panel_set_gir_off(struct drm_panel *panel)
{
	...
	if (ctx->dynamic_fps == 144) {
		mi_disp_panel_ddic_send_cmd(gir_off_144hz_settings,
					    ARRAY_SIZE(gir_off_144hz_settings),
					    false, false);
	} else {
		mi_disp_panel_ddic_send_cmd(
			gir_off_not_144hz_settings,
	...
	for (i = 0; i < 6; i += 2) {
		...
		if (panel_build_id == PANEL_APL2600 ||
			...
			temp = (int)lhbm_cmdbuf[i / 2 + 6] *
			       gamma_coffee_w900_gir_off_peak2600[i / 2] /
			       10000;
		} else {
			temp = (((int)lhbm_cmdbuf[i / 2 + 6]) *
				gamma_coffee_w900_gir_off[i / 2]) /
			       1000;
		}
						 ...
						 int flat_mode)
{
	...
	*/
	...
	case TYPE_WHITE_1200:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_normal_white_1200nit[4].para_list[i] =
	...
	case TYPE_WHITE_250:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_normal_white_250nit[4].para_list[i] =
	...
	case TYPE_GREEN_500:
		if (flat_mode) {
			for (i = 0; i < 2; i++) {
				lhbm_normal_green_500nit[4].para_list[i + 2] =
	...
	case TYPE_HLPM_W1200:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_hlpm_white_1200nit[12].para_list[i] =
					lhbm_whitebuf.gir_on_900[i];
			}
	...
	case TYPE_HLPM_W250:
		if (flat_mode) {
			for (i = 0; i < 6; i++) {
				lhbm_hlpm_white_250nit[12].para_list[i] =
					lhbm_whitebuf.gir_on_110[i];
			}
...
static void mi_disp_panel_update_lhbm_backlight(struct mtk_dsi *dsi,
						enum lhbm_cmd_type type,
						int bl_level)
{
...
static void mi_disp_panel_update_lhbm_alpha(struct mtk_dsi *dsi,
					    enum lhbm_cmd_type type,
					    int bl_level)
{
	u8 alpha_buf[2] = { 0 };
	...
	case TYPE_WHITE_1200:
		if (bl_level < 0x146) {
			...
			lhbm_normal_white_1200nit[9].para_list[1] =
				alpha_buf[0];
			lhbm_normal_white_1200nit[9].para_list[2] =
				alpha_buf[1];
			...
			lhbm_normal_white_1200nit[7].para_list[0] =
				alpha_buf[0];
			lhbm_normal_white_1200nit[7].para_list[1] =
				alpha_buf[1];
			      ...
			      enum local_hbm_state lhbm_state)
{
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT:
		...
		if (atomic_read(&doze_enable)) {
			if (bl_level)
				mi_disp_panel_update_lhbm_alpha(
					dsi, TYPE_LHBM_OFF, bl_level);
			else
				mi_disp_panel_update_lhbm_alpha(
					dsi, TYPE_LHBM_OFF, bl_level_doze);
		} else {
			mi_disp_panel_update_lhbm_alpha(dsi, TYPE_LHBM_OFF,
							bl_level);
		}
		mi_disp_panel_ddic_send_cmd(lhbm_off, ARRAY_SIZE(lhbm_off),
					    false, true);
	...
	case LOCAL_HBM_NORMAL_WHITE_110NIT:
		...
		if (atomic_read(&doze_enable)) {
			mi_disp_panel_update_lhbm_alpha(dsi, TYPE_HLPM_W250,
							bl_level_doze);
			mi_disp_panel_update_lhbm_white_param(
				dsi, TYPE_HLPM_W250, flat_mode);
			mi_disp_panel_update_lhbm_backlight(
				dsi, TYPE_HLPM_W1200, bl_level_doze);
			mi_disp_panel_ddic_send_cmd(
			...
			mi_disp_panel_update_lhbm_alpha(dsi, TYPE_WHITE_250,
							bl_level);
			mi_disp_panel_update_lhbm_white_param(
	...
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		if (atomic_read(&doze_enable)) {
			mi_disp_panel_update_lhbm_alpha(dsi, TYPE_HLPM_W1200,
							bl_level_doze);
			mi_disp_panel_update_lhbm_white_param(
			...
			mi_disp_panel_update_lhbm_alpha(dsi, TYPE_WHITE_1200,
							bl_level);
			mi_disp_panel_update_lhbm_white_param(
	...
	case LOCAL_HBM_HLPM_WHITE_1000NIT:
		...
		mi_disp_panel_update_lhbm_backlight(dsi, TYPE_HLPM_W1200,
						    bl_level_doze);
