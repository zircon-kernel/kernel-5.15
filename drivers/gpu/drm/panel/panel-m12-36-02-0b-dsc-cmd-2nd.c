...
static unsigned int m12_36_dphy_range_min_qp[15] = { 0x0, 0x0, 0x5, 0x5, 0x7,
						     0x7, 0x7, 0x7, 0x7, 0x8,
						     0x9, 0x9, 0x9, 0xc, 0x10 };
static unsigned int m12_36_dphy_range_max_qp[15] = {
	0x0, 0x2, 0x9, 0xa, 0xb, 0xb,  0xb, 0xc,
	0xd, 0xe, 0xe, 0xf, 0xf, 0x10, 0x11
};
static unsigned int m12_36_dphy_range_bpg_ofs[15] = { 0x24, 0x4,  0x0,	0x3e,
						      0x3c, 0x3a, 0x38, 0x38,
						      0x38, 0x36, 0x36, 0x36,
						      0x34, 0x34, 0x34 };
...
static int lcm_enable(struct drm_panel *panel)
{
	...
	if (ctx->mode_index != -1 && ctx->connector != NULL) {
		...
		struct drm_display_mode *mode =
			get_mode_by_id(ctx->connector, ctx->mode_index);
		mtk_panel_ext_param_get(panel, ctx->connector,
					&panel_params_dst, ctx->mode_index);

		iris_update_2nd_active_timing(
				   ...
				   unsigned int mode)
{
	...
	if (m->vdisplay == VACT_RAW)
		...
		pr_err("%s cannot get mode: %d, vrefresh = %d, hdisplay = %d, vdisplay = %d",
		       __func__, mode, drm_mode_vrefresh(m), m->hdisplay,
		       m->vdisplay);
		return ret;
	}
		       ...
		       enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	pr_info("%s isFpsChange = %d, isResChange = %d\n", __func__,
		isFpsChange, isResChange);
	pr_info("%s dst_mode vrefresh = %d, vdisplay = %d, vdisplay = %d, dsc: %d\n",
		__func__, drm_mode_vrefresh(m_dst), m_dst->vdisplay,
		m_dst->hdisplay, panel_params_dst->dsc_params.enable);
	pr_info("%s cur_mode vrefresh = %d, vdisplay = %d, vdisplay = %d, dsc: %d\n",
		__func__, drm_mode_vrefresh(m_cur), m_cur->vdisplay,
		m_cur->hdisplay, panel_params_cur->dsc_params.enable);
			 ...
			 struct drm_connector *connector)
{
	...
	mode3 = drm_mode_duplicate(connector->dev,
				   &performence_mode_144); //144Hz
	if (!mode1) {
		dev_err(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			performence_mode.hdisplay,
	...
	if (!mode4) {
		dev_info(connector->dev->dev, "failed to add mode %ux%ux@%u\n",
			 performance_mode_raw_120hz.hdisplay,
			 performance_mode_raw_120hz.vdisplay,
	...
	pr_info("%s- m12-36-02-0b-dsc 2nd \n", __func__);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (!is_mi_dev_support_iris()) {
		pr_err("m12-36-02-0b-dsc 2nd : IRIS:mi dev not support iris\n");
		return -ENODEV;
	}
	...
	if (dsi_node) {
		...
		remote_node =
			of_find_compatible_node(NULL, NULL, secondary_panel);
		if (!remote_node) {
	...
	pr_info("%s m12-36-02-0b-dsc 2nd DSC 120Hz\n", __func__);
	ctx->mode_index = -1;
	ctx->connector = NULL;
	return ret;
}
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "m12_36_02_0b_dsc_cmd,lcm,2nd",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("m12_36_02_0b_dsc_cmd_2nd oled panel driver");
