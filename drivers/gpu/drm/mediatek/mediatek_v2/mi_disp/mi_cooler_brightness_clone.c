...
static struct thermal_cooling_device *cl_backlight_dev[BACKLIGHT_COOLER_NR] = {
	0
};

static int
mi_cooler_brightness_clone_get_max_state(struct thermal_cooling_device *cdev,
					 unsigned long *state)
{
...
static int
mi_cooler_brightness_clone_get_cur_state(struct thermal_cooling_device *cdev,
					 unsigned long *state)
{
...
static int
mi_cooler_brightness_clone_set_cur_state(struct thermal_cooling_device *cdev,
					 unsigned long state)
{
	struct mtk_dsi *dsi = NULL;
	...
	thermal_limit_brightness_clone =
		max_brightness_clone * state / MAX_THERMAL_STATE;
...
static void mi_cooler_brightness_clone_unregister_ltf(void)
{
	...
	if (cl_backlight_dev[0]) {
		thermal_cooling_device_unregister(cl_backlight_dev[0]);
