...
#define DISPLOG_WARN(fmt, ...) disp_log_printk("[warn]:" fmt, ##__VA_ARGS__)
#define DISPLOG_ERR(fmt, ...) disp_log_printk("[error]:" fmt, ##__VA_ARGS__)
#define DISPLOG_INFO(fmt, ...) disp_log_printk("[info]:" fmt, ##__VA_ARGS__)
#define DISPLOG_DEBUG(fmt, ...) disp_log_printk("[debug]:" fmt, ##__VA_ARGS__)
...
#else
static inline int mi_disp_log_init(void)
{
	return 0;
}
