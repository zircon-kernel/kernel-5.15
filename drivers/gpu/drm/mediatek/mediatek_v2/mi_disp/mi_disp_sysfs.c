			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return mi_dsi_display_get_disp_param(dd_ptr->display, buf,
						     PAGE_SIZE);
	} else {
			     ...
			     size_t count)
{
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		ret = mi_dsi_display_write_mipi_reg(dd_ptr->display,
						    (char *)buf);
	} else {
		DISP_ERROR("Unsupported display(%s intf)\n",
			   get_disp_intf_type_name(dd_ptr->intf_type));
...
static ssize_t gir_store(struct device *device, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		if (strstr(buf, "on")) {
			ret = mi_dsi_display_enable_gir(dd_ptr->display,
							(char *)buf);
		} else if (strstr(buf, "off")) {
			ret = mi_dsi_display_disable_gir(dd_ptr->display,
							 (char *)buf);
		}
	} else {
		DISP_ERROR("Unsupported display(%s intf)\n",
...
static ssize_t gir_show(struct device *device, struct device_attribute *attr,
			char *buf)
{
	struct disp_display *dd_ptr = to_disp_display(device);
			    ...
			    struct device_attribute *attr, char *buf)
{
	...
	struct disp_display *dd_ptr = to_disp_display(device);
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return mi_dsi_display_read_wp_info(dd_ptr->display, buf,
						   wpinfo_size);
	} else {
		return snprintf(buf, PAGE_SIZE,
				"Unsupported display(%s intf)\n",
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	struct disp_display *dd_ptr = to_disp_display(device);
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return mi_dsi_display_read_grayscale_info(dd_ptr->display, buf,
							  buf_size);
	} else {
		return snprintf(buf, PAGE_SIZE,
				"Unsupported display(%s intf)\n",
					 ...
					 char *buf)
{
	...
	mi_dsi_display_get_max_brightness_clone(dd_ptr->display,
					   ...
					   char *buf)
{
	...
	mi_dsi_display_get_factory_max_brightness(dd_ptr->display,
...
static ssize_t idle_store(struct device *device, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	int ret = 0;
...
static ssize_t idle_show(struct device *device, struct device_attribute *attr,
			 char *buf)
{
	struct disp_display *dd_ptr = to_disp_display(device);
...
ssize_t mi_drm_sysfs_read_panel_info(struct drm_connector *connector, char *buf)
{
