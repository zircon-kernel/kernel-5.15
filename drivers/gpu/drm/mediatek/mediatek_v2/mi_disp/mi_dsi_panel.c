...
ssize_t dsi_panel_write_mipi_reg(char *buf, size_t count)
{
	...
	pr_info("[%s]: mipi_write_date source: count = %d,buf = %s ", __func__,
		(int)count, buf);
	...
	if (lcm_mipi_read_write.read_enable) {
		...
		lcm_mipi_read_write.lcm_setting_table.count =
		...
		if (count > 8) {
			...
			memcpy(lcm_mipi_read_write.lcm_setting_table.para_list,
			       data, dlen);
	...
	pr_debug("[%s]: write cmd = %d,len = %d\n", __func__,
		 lcm_mipi_read_write.lcm_setting_table.cmd,
		 lcm_mipi_read_write.lcm_setting_table.count);
	pr_debug("[%s]: mipi_write data: ", __func__);
	for (i = 0; i < count - 3; i++) {
		pr_debug("0x%x ",
			 lcm_mipi_read_write.lcm_setting_table.para_list[i]);
	}
	pr_debug("\n ");
...
static int mi_dsi_panel_get_cmd_pkt_count(const char *data, u32 length,
					  u32 *cnt)
{
...
static int mi_dsi_panel_create_cmd_packets(const char *data, u32 count,
...
ssize_t mi_dsi_panel_enable_gir(struct mtk_dsi *dsi, char *buf)
{
	...
	panel_ext = mtk_dsi_get_panel_ext(comp);
	if (!(panel_ext && panel_ext->funcs &&
	      panel_ext->funcs->panel_set_gir_on)) {
...
static void panel_gir_off_control(struct mtk_dsi *dsi)
{
	...
	panel_ext = mtk_dsi_get_panel_ext(comp);
	if (!(panel_ext && panel_ext->funcs &&
	      panel_ext->funcs->panel_set_gir_off)) {
...
{
	struct mtk_dsi *dsi =
		container_of(work, struct mtk_dsi, gir_off_delayed_work.work);
	if (!dsi) {
		DISP_ERROR("invalid params\n");
		return;
	}
...
int mi_dsi_panel_get_gir_status(struct mtk_dsi *dsi)
{
	...
	panel_ext = mtk_dsi_get_panel_ext(comp);
	if (panel_ext && panel_ext->funcs &&
	    panel_ext->funcs->panel_get_gir_status) {
...
ssize_t mi_dsi_panel_write_mipi_reg(char *buf)
{
	...
	DISP_INFO("cmd_msg->flags= 0x%02x\n", cmd_msg->flags);
	...
	if (lcm_mipi_read_write.read_enable) {
		...
		if (lcm_mipi_read_write.read_count > 10) {
			...
			while (read_length > 0) {
				...
				memset(temp_read_buffer, 0,
				       sizeof(temp_read_buffer));
				cmd_msg->rx_len[0] =
					read_length > 10 ? 10 : read_length;
				rc = mtk_ddic_dsi_read_cmd(cmd_msg);
				if (rc != 0) {
					pr_err("%s error\n", __func__);
				}
				memcpy(&lcm_mipi_read_write.read_buffer
						[read_buffer_position],
				       temp_read_buffer, cmd_msg->rx_len[0]);
				read_buffer_position += cmd_msg->rx_len[0];
				read_length -= 10;
			}
		} else {
			cmd_msg->rx_buf[0] = lcm_mipi_read_write.read_buffer;
			memset(cmd_msg->rx_buf[0], 0,
		...
		rc = mi_dsi_panel_get_cmd_pkt_count(buffer, buf_size,
						    &packet_count);
		if (!packet_count) {
		...
		cmds = kzalloc(sizeof(struct dsi_cmd_desc) * packet_count,
			       GFP_KERNEL);
		if (!cmds) {
			rc = -ENOMEM;
			goto exit_free0;
		...
		rc = mi_dsi_panel_create_cmd_packets(buffer, packet_count,
						     cmds);
		if (rc) {
			DISP_ERROR(
				"panel failed to create cmd packets, rc=%d\n",
				rc);
			goto exit_free2;
		}
		...
		for (i = 0; i < packet_count; i++) {
			...
			for (j = 0; j < cmds[i].msg.tx_len; j++) {
				pr_info("send lcm type[%d]=0x%x, tx_buf[%d]--byte:%d,val:%x\n",
...
ssize_t mi_dsi_panel_read_mipi_reg_cp(char *exitbuf, char *enterbuf,
				      int read_buf_pos)
{
	int i = 0;
	int j = 0;
	...
	if (lcm_mipi_read_write.read_enable) {
		for (i = read_buf_pos;
		     i < read_buf_pos + lcm_mipi_read_write.read_count; i++) {
...
void mi_dsi_panel_rewrite_enterDClut(char *exitDClut, char *enterDClut,
				     int count)
{
	int i = 0, j = 0;
...
static int mi_get_dc_lut(char *page4buf, char *offsetbuf, char *read_exitDClut,
			 char *exitDClut, char *enterDClut, int read_buf_pos)
{
	...
	write_count =
		dsi_panel_write_mipi_reg(offsetbuf, strlen(offsetbuf) + 1);
	if (!write_count) {
		DISP_ERROR("DC LUT switch to offsetbuf failed\n");
		goto end;
	}
	write_count = dsi_panel_write_mipi_reg(read_exitDClut,
					       strlen(read_exitDClut) + 1);
	if (!write_count) {
		DISP_ERROR("DC LUT switch to read_exitDClut failed\n");
		goto end;
	}
	read_count = mi_dsi_panel_read_mipi_reg_cp(exitDClut, enterDClut,
						   read_buf_pos);
...
ssize_t mi_dsi_panel_read_and_update_dc_param(struct mtk_dsi *dsi)
{
	...
	while (read_length > 0) {
		...
		offset_count = snprintf(offsetbuf + strlen(offsetbuf) - 2, 3,
					"%02x", read_offset);
		DISP_DEBUG(
			"offsetbuf = %s strlen(offsetbuf) = %d offset_count = %d\n",
			offsetbuf, strlen(offsetbuf), offset_count);
		offset_count = snprintf(read_buf, 3, "%02x", len);
		DISP_DEBUG(
			"read_buf = %s strlen(read_buf) = %d offset_count = %d len = %d\n",
		...
		DISP_DEBUG(
			"read_exitDClut60 = %s strlen(read_exitDClut60) = %d offset_count = %d len = %d\n",
			read_exitDClut60, strlen(read_exitDClut60),
			offset_count, len);
		DISP_DEBUG(
			"read_exitDClut120 = %s strlen(read_exitDClut120) = %d offset_count = %d len = %d\n",
			read_exitDClut120, strlen(read_exitDClut120),
			offset_count, len);
		count = mi_get_dc_lut(page4buf, offsetbuf, read_exitDClut60,
				      exitDClut60, enterDClut60, read_buf_pos);
		if (count < 0) {
			DISP_ERROR("mi_get_dc_lut 60hz count = %d\n", count);
			goto end;
		}
		count = mi_get_dc_lut(page4buf, offsetbuf, read_exitDClut120,
				      exitDClut120, enterDClut120,
				      read_buf_pos);
		if (count < 0) {
	...
	DISP_DEBUG(
		"mi_dsi_panel_read_and_update_dc_param start read_buf_pos = %d 60hz\n",
		read_buf_pos);
	mi_dsi_panel_rewrite_enterDClut(exitDClut60, enterDClut60,
					read_buf_pos);
	DISP_DEBUG(
		"mi_dsi_panel_read_and_update_dc_param start read_buf_pos = %d 120hz\n",
		read_buf_pos);
	mi_dsi_panel_rewrite_enterDClut(exitDClut120, enterDClut120,
					read_buf_pos);

	if (!(panel_ext && panel_ext->funcs &&
	      panel_ext->funcs->panel_set_dc_lut_params)) {
		pr_info("%s panel_set_dc_lut_params func not defined");
		goto end;
	} else {
		panel_ext->funcs->panel_set_dc_lut_params(
			dsi->panel, exitDClut60, enterDClut60, exitDClut120,
			enterDClut120, read_buf_pos);
	}
end:
	DISP_INFO("-\n");
	return 0;
...
int mi_dsi_panel_write_dsi_cmd(struct dsi_cmd_rw_ctl *ctl)
{
	...
	rc = mi_dsi_panel_get_cmd_pkt_count(ctl->tx_ptr, ctl->tx_len,
					    &packet_count);
	if (!packet_count) {
		DISP_ERROR("get pkt count failed!\n");
		goto exit;
...
static int dsi_panel_get_lockdown_from_cmdline(unsigned char *plockdowninfo)
{
	...
	if (sscanf(lockdown_info,
		   "0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx",
		   &plockdowninfo[0], &plockdowninfo[1], &plockdowninfo[2],
		   &plockdowninfo[3], &plockdowninfo[4], &plockdowninfo[5],
		   &plockdowninfo[6], &plockdowninfo[7]) != 8) {
		pr_err("failed to parse lockdown info from cmdline !\n");
	} else {
		if (plockdowninfo[0] == 0 && plockdowninfo[1] == 0 &&
		    plockdowninfo[2] == 0 && plockdowninfo[3] == 0 &&
		    plockdowninfo[4] == 0 && plockdowninfo[5] == 0 &&
		    plockdowninfo[6] == 0 && plockdowninfo[7] == 0) {
		...
		pr_info("lockdown info from cmdline = 0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,0x%02hhx,"
			...
			plockdowninfo[0], plockdowninfo[1], plockdowninfo[2],
			plockdowninfo[3], plockdowninfo[4], plockdowninfo[5],
			plockdowninfo[6], plockdowninfo[7]);
...
ssize_t led_i2c_reg_write(struct mtk_dsi *dsi, char *buf, unsigned long count)
{
	...
	if (count < 9 || buf == NULL) {
		...
		pr_info("[%s], command is invalid, count  = %ld,buf = %s ",
			__func__, count, buf);
		return retval;
	}
	...
	if (lcm_led_i2c_read_write.read_enable) {
		...
		memset(lcm_led_i2c_read_write.buffer, 0,
		       sizeof(lcm_led_i2c_read_write.buffer));
		lcm_led_i2c_read_write.buffer[0] = (unsigned char)register_addr;
		if (panel_ext->funcs->led_i2c_reg_op)
			retval = panel_ext->funcs->led_i2c_reg_op(
				lcm_led_i2c_read_write.buffer,
				KTZ8863A_REG_READ,
				lcm_led_i2c_read_write.read_count);
	} else {
		if (count < 12)
			return retval;
		...
		memcpy(pbuf, input, 2);
		pbuf[2] = '\0';
		lcm_led_i2c_read_write.buffer[1] =
			(unsigned char)string_to_hex(pbuf);

		if (panel_ext->funcs->led_i2c_reg_op)
			retval = panel_ext->funcs->led_i2c_reg_op(
				lcm_led_i2c_read_write.buffer,
				KTZ8863A_REG_WRITE, 0);
	}
					   ...
					   int speed)
{
	...
	if (speed != 0 && speed != MIPI_DSI_MSG_USE_LPM) {
		pr_err("speed invalid, value:%d, use high speed as default \n",
		       speed);
		cmd_msg.flags = 0;
	}

	pr_info("%s wait_te:%d , conut:%d\n", __func__, wait_te_send, count);
	for (i = 0; i < count; i++) {
...
EXPORT_SYMBOL(mi_disp_panel_ddic_send_cmd_with_speed);

int mi_disp_panel_ddic_send_cmd(struct LCM_setting_table *table,
				unsigned int count, bool wait_te_send,
				bool block)
{
	int i = 0, j = 0, k = 0;
	int ret = 0;
...
int mi_dsi_panel_get_panel_info(struct mtk_dsi *dsi, char *buf)
{
	...
	if (!(panel_ext && panel_ext->funcs &&
	      panel_ext->funcs->get_panel_info)) {
...
int mi_dsi_panel_get_wp_info(struct mtk_dsi *dsi, char *buf, size_t size)
{
	...
	if (panel_ext && panel_ext->funcs && panel_ext->funcs->get_wp_info) {
		if (mtk_drm_lcm_is_connect(mtk_crtc))
			count = panel_ext->funcs->get_wp_info(dsi->panel, buf,
							      size);
		else {
...
int mi_dsi_panel_get_grayscale_info(struct mtk_dsi *dsi, char *buf, size_t size)
{
	...
	panel_ext = mtk_dsi_get_panel_ext(comp);
	if (panel_ext && panel_ext->funcs &&
	    panel_ext->funcs->get_grayscale_info) {
		count = panel_ext->funcs->get_grayscale_info(dsi->panel, buf,
							     size);
	}
...
int mi_dsi_panel_get_fps(struct mtk_dsi *dsi, u32 *fps)
{
	struct mtk_ddp_comp *comp = NULL;
	struct mtk_panel_ext *panel_ext = NULL;
	...
	if (!(panel_ext && panel_ext->funcs &&
	      panel_ext->funcs->get_panel_dynamic_fps)) {
					  ...
					  u32 *max_brightness_clone)
{
	...
	if (!(panel_ext && panel_ext->funcs &&
	      panel_ext->funcs->get_panel_max_brightness_clone)) {
		pr_info("%s get_panel_max_brightness_clone func not defined",
			__func__);
		return -1;
	} else {
		return panel_ext->funcs->get_panel_max_brightness_clone(
			dsi->panel, max_brightness_clone);
	}
}
					    ...
					    u32 *max_brightness_clone)
{
	...
	if (!(panel_ext && panel_ext->funcs &&
	      panel_ext->funcs->get_panel_factory_max_brightness)) {
		pr_info("%s get_panel_max_brightness_clone func not defined",
			__func__);
		return -1;
	} else {
		return panel_ext->funcs->get_panel_factory_max_brightness(
			dsi->panel, max_brightness_clone);
	}
}
...
int mi_dsi_panel_set_doze_brightness(struct mtk_dsi *dsi, int doze_brightness)
{
	...
	if (dsi->mi_cfg.lhbm_en && doze_brightness) {
		if (panel_ext && panel_ext->params &&
		    panel_ext->params->aod_delay_enable) {
			if (doze_brightness && !(dsi->mi_cfg.aod_wait_frame)) {
				ret = wait_for_completion_timeout(
					&dsi->aod_wait_completion,
		...
		if (panel_ext && panel_ext->funcs &&
		    panel_ext->funcs->set_only_aod_backlight)
			panel_ext->funcs->set_only_aod_backlight(
				dsi->panel, doze_brightness);
	...
	if (!(panel_ext && panel_ext->funcs &&
		...
		mi_disp_feature_event_notify_by_type(
			mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
			sizeof(doze_brightness),
			doze_brightness + DOZE_BRIGHTNESS_MAX);
#endif
		if (panel_ext && panel_ext->params &&
		    panel_ext->params->aod_delay_enable) {
			if (doze_brightness && !(dsi->mi_cfg.aod_wait_frame)) {
		...
		if (!dsi->mi_cfg.fod_anim_flag && !doze_brightness &&
		    dsi->mi_cfg.dynamic_fps == 120) {
			panel_ext->funcs->set_only_aod_backlight(dsi->panel, 0);
		}

		if (!panel_ext->funcs->set_doze_brightness(dsi->panel,
							   doze_brightness)) {
			mi_disp_feature_event_notify_by_type(
				mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
				sizeof(doze_brightness), doze_brightness);
			dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] =
				doze_brightness;
#if IS_ENABLED(CONFIG_DRM_PANEL_N16U_42_02_0A_DSC_VDO)
			...
			mi_disp_feature_event_notify_by_type(
				mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
				sizeof(doze_brightness), DOZE_TO_NORMAL);
			dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] =
				DOZE_TO_NORMAL;
		}
	}
...
int mi_dsi_panel_get_doze_brightness(struct mtk_dsi *dsi, u32 *doze_brightness)
{
	if (!dsi) {
		DISP_ERROR("invalid params\n");
		return -EINVAL;
...
int mi_dsi_panel_set_brightness(struct mtk_dsi *dsi, int brightness)
{
	int ret = 0;
	struct mtk_ddp_comp *comp = NULL;
	struct mtk_panel_ext *panel_ext = NULL;
	...
	if (!(panel_ext && panel_ext->funcs &&
		...
		if (!panel_ext->funcs->setbacklight_control(dsi->panel,
							    brightness)) {
			mi_disp_feature_event_notify_by_type(
				mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
...
int mi_dsi_panel_get_brightness(struct mtk_dsi *dsi, u32 *brightness)
{
	if (!dsi) {
		DISP_ERROR("invalid params\n");
		return -EINVAL;
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi,
				struct disp_feature_ctl *ctl)
{
	...
	if (dsi->encoder.crtc && dsi->encoder.crtc->dev &&
	    dsi->encoder.crtc->dev->dev_private)
	...
	case DISP_FEATURE_DIMMING:
		...
		if (ctl->feature_val == FEATURE_ON)
			...
			panel_ext->funcs->panel_elvss_control(dsi->panel,
							      false);
	...
	case DISP_FEATURE_HBM:
		...
		if (panel_ext->funcs->normal_hbm_control) {
			if (ctl->feature_val == FEATURE_ON) {
				panel_ext->funcs->normal_hbm_control(dsi->panel,
								     1);
	...
	case DISP_FEATURE_HBM_FOD:
		...
		if (panel_ext->funcs->hbm_fod_control) {
			if (ctl->feature_val == FEATURE_ON) {
				panel_ext->funcs->hbm_fod_control(dsi->panel,
								  true);
			} else {
				panel_ext->funcs->hbm_fod_control(dsi->panel,
								  false);
	...
	case DISP_FEATURE_DOZE_BRIGHTNESS:
		...
		if (is_support_doze_brightness(ctl->feature_val)) {
			panel_ext->funcs->set_doze_brightness(dsi->panel,
							      ctl->feature_val);
		...
		if (is_support_doze_brightness(ctl->feature_val)) {
			...
			} else if (ctl->feature_val != DOZE_TO_NORMAL &&
				   mi_cfg->feature_val
	...
	case DISP_FEATURE_FOD_CALIBRATION_BRIGHTNESS:
		if (ctl->feature_val == -1) {
			...
			if (panel_ext->funcs->setbacklight_control)
				panel_ext->funcs->setbacklight_control(
					dsi->panel, mi_cfg->last_bl_level);
	...
	case DISP_FEATURE_FOD_CALIBRATION_HBM:
		if (ctl->feature_val == -1) {
			DISP_INFO(
				"FOD calibration HBM restore last_bl_level=%d\n",
				mi_cfg->last_bl_level);
			if (panel_ext->funcs->hbm_fod_control)
				panel_ext->funcs->hbm_fod_control(dsi->panel,
								  false);
		...
		mi_cfg->feature_val[DISP_FEATURE_FOD_CALIBRATION_HBM] =
			ctl->feature_val;
		break;
	...
	case DISP_FEATURE_LOW_BRIGHTNESS_FOD:
		DISP_INFO("DISP_FEATURE_LOW_BRIGHTNESS_FOD=%d\n",
			  ctl->feature_val);
		mi_cfg->feature_val[DISP_FEATURE_LOW_BRIGHTNESS_FOD] =
			ctl->feature_val;
		break;
	...
	IS_ENABLED(CONFIG_DRM_PANEL_L2M_38_0A_0A_DSC_CMD)
			...
			mtk_ddp_comp_io_cmd(comp, NULL, MI_RESTORE_CRC_LEVEL,
					    &need_lock);
			mtk_ddp_comp_io_cmd(comp, NULL, SET_DC_SYNC_TE_MODE_ON,
					    NULL);
#endif
#endif
		} else {
	...
	IS_ENABLED(CONFIG_DRM_PANEL_L2M_38_0A_0A_DSC_CMD)
#ifdef CONFIG_MI_DISP_SILKY_BRIGHTNESS_CRC
			mtk_ddp_comp_io_cmd(comp, NULL, SET_DC_SYNC_TE_MODE_OFF,
					    NULL);
#endif
#endif
		}
	...
	case DISP_FEATURE_CRC:
		...
		case CRC_SRGB: {
			if (!panel_ext->funcs->panel_set_crc_srgb)
				break;
			if (mi_cfg->feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] !=
			    DOZE_TO_NORMAL) {
	...
	case DISP_FEATURE_GIR:
		if (panel_ext && panel_ext->funcs &&
		    panel_ext->funcs->panel_get_gir_status) {
			if (panel_ext->funcs->panel_get_gir_status(
				    dsi->panel)) {
...
#if IS_ENABLED(CONFIG_DRM_PANEL_N16U_42_02_0A_DSC_VDO)
		if (ctl->feature_val == AUTH_STOP &&
		    (mi_cfg->user_doze_brightness || mi_cfg->doze_state)) {
			if (panel_ext && panel_ext->funcs &&
			    panel_ext->funcs->set_only_aod_backlight)
				panel_ext->funcs->set_only_aod_backlight(
					dsi->panel, 0);
		...
		if (is_support_lcd_hbm_level(ctl->feature_val)) {
			...
			if (panel_ext->funcs->normal_hbm_control) {
				panel_ext->funcs->normal_hbm_control(
					dsi->panel, ctl->feature_val);
				dsi->mi_cfg.normal_hbm_flag =
					ctl->feature_val == FEATURE_ON ? true :
									 false;
			}
			mi_cfg->feature_val[DISP_FEATURE_LCD_HBM] =
				ctl->feature_val;
		} else
	...
	case DISP_FEATURE_SPR_RENDER:
		DISP_INFO("SPR:%d\n", ctl->feature_val);
		if (mi_cfg->feature_val[DISP_FEATURE_SPR_RENDER] ==
		    ctl->feature_val) {
			DISP_INFO("SPR is the same, return\n");
			break;
		}
		if (panel_ext->funcs->set_spr_status) {
			panel_ext->funcs->set_spr_status(dsi->panel,
							 ctl->feature_val);
		}
	...
	case DISP_FEATURE_POWERSTATUS:
		...
		if (dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] ==
			    DOZE_TO_NORMAL &&
		    panel_ext->funcs->set_only_aod_backlight &&
		    ctl->feature_val == 0) {
			panel_ext->funcs->set_only_aod_backlight(dsi->panel, 0);
	...
	case DISP_FEATURE_PEAK_HDR_MODE:
		if (panel_ext->funcs->set_peak_hdr_status) {
			panel_ext->funcs->set_peak_hdr_status(dsi,
							      ctl->feature_val);
...
ssize_t mi_dsi_panel_get_disp_param(struct mtk_dsi *dsi, char *buf, size_t size)
{
	...
	for (i = DISP_FEATURE_DIMMING; i < DISP_FEATURE_MAX; i++) {
		count +=
			snprintf(buf + count, size - count, "%036s[%02d]: %d\n",
...
void mi_disp_cfg_init(struct mtk_dsi *dsi)
{
	...
	ret = mi_dsi_panel_get_max_brightness_clone(
		dsi, &dsi->mi_cfg.max_brightness_clone);
	if (!ret) {
		dsi->mi_cfg.thermal_max_brightness_clone =
		...
		dsi->mi_cfg.thermal_max_brightness_clone =
			DEFAULT_MAX_BRIGHTNESS_CLONE;
		DISP_INFO("default max brightness clone:%d\n",
			  dsi->mi_cfg.max_brightness_clone);
	}

	ret = mi_dsi_panel_get_factory_max_brightness(
		dsi, &dsi->mi_cfg.factory_max_brightness);
	if (!ret) {
...
MODULE_PARM_DESC(
	panel_opt,
	"mediatek_drm.panel_opt=<panel_opt> while <panel_opt> is 'panel_opt' ");
