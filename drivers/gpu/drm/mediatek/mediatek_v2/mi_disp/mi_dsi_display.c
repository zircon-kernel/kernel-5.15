...
ssize_t mi_dsi_display_get_disp_param(void *display, char *buf, size_t size)
{
	struct mtk_dsi *dsi = (struct mtk_dsi *)display;
...
ssize_t mi_dsi_display_write_led_i2c_reg(void *display, char *buf, size_t count)
{
	struct mtk_dsi *dsi = (struct mtk_dsi *)display;
	if (!dsi) {
		DISP_ERROR("Invalid display ptr\n");
...
ssize_t mi_dsi_display_read_led_i2c_reg(void *display, char *buf)
{
	struct mtk_dsi *dsi = (struct mtk_dsi *)display;
	if (!dsi) {
		DISP_ERROR("Invalid display ptr\n");
...
ssize_t mi_dsi_display_read_panel_info(void *display, char *buf)
{
...
ssize_t mi_dsi_display_read_grayscale_info(void *display, char *buf,
					   size_t size)
{
	struct mtk_dsi *dsi_display = (struct mtk_dsi *)display;
	if (!dsi_display) {
...
int mi_dsi_display_get_doze_brightness(void *display, u32 *doze_brightness)
{
	...
	return mi_dsi_panel_get_doze_brightness(dsi_display, doze_brightness);
}
...
int mi_dsi_display_get_brightness(void *display, u32 *brightness)
{
	...
	return mi_dsi_panel_get_brightness(dsi_display, brightness);
}
						      ...
						      u32 brightness_clone)
{
	...
	if (dsi_display->mi_cfg.real_brightness_clone == -1) {
		DISP_INFO(
			"thermal set cur_state early, not brightness_clone set record\n");
		mi_dsi_display_set_brightness_clone(
			dsi_display,
			(dsi_display->mi_cfg.max_brightness_clone + 1) / 4);
	} else {
		mi_dsi_display_set_brightness_clone(
			dsi_display, dsi_display->mi_cfg.real_brightness_clone);
	}
	return 0;
}

int mi_dsi_display_get_thermal_limit_brightness_clone(
	void *display, u32 *thermal_limit_brightness_clone)
{
	struct mtk_dsi *dsi_display = (struct mtk_dsi *)display;
