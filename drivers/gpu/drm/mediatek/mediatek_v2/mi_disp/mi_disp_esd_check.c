...
static int mi_disp_request_err_flag(struct drm_crtc *crtc)
{
	...
	if (gpio_is_valid(mi_esd_ctx->err_flag_irq_gpio)) {
		mi_esd_ctx->err_flag_irq =
			gpio_to_irq(mi_esd_ctx->err_flag_irq_gpio);
		ret = gpio_request(mi_esd_ctx->err_flag_irq_gpio,
				   "esd_err_irq_gpio");
		if (ret) {
	...
	if (mi_esd_ctx->err_flag_irq_gpio > 0) {
		ret = request_threaded_irq(mi_esd_ctx->err_flag_irq, NULL,
					   _mi_esd_check_err_flag_irq_handler,
...
static void mi_disp_esd_err_flag_irq_init(struct mi_esd_ctx *mi_esd_ctx,
...
int mi_disp_esd_irq_ctrl(struct mi_esd_ctx *mi_esd_ctx, bool enable)
{
