				   ...
				   va_list args)
{
	...
	ts_usec %= 1000000;
	text_len = snprintf(text, sizeof(textbuf), "[%5lld.%06lld] ", ts_sec,
			    ts_usec);
				  ...
				  struct disp_log_read *read_ptr, size_t count)
{
	...
	while (log_start->offset == log->pos.offset) {
		...
		if (wait_event_freezable(log->wq_head,
					 atomic_read(&log->wait) == 0)) {
				...
				size_t count, loff_t *ppos)
{
	...
	size_t max_len =
		log_buf->pos.wrap ? log_buf->size : log_buf->pos.offset;
	size_t read_len = 0;
	ssize_t ret;
	...
	if (log_buf->pos.wrap) {
		if (log_buf->pos.offset + read_len < log_buf->size) {
			...
			if (copy_to_user(buf + log_buf->size -
						 log_buf->pos.offset,
					 &log_buf->buf[log_buf->pos.offset],
					 read_len - (log_buf->size -
						     log_buf->pos.offset))) {
				ret = -EFAULT;
				goto out;
			}
