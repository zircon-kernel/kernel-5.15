...
int mi_drm_bl_wait_for_completion(struct drm_crtc *crtc, unsigned int level)
{
	...
	if (is_backlight_set_block(dsi, level)) {
		...
		ret = wait_for_completion_timeout(&dsi->bl_wait_completion,
...
static int mi_drm_update_aod_status(struct mtk_dsi *dsi, bool doze_state,
				    bool is_aod_exit)
{
	...
	if (is_aod_exit && !doze_state) {
		if (!mi_cfg->bl_enable &&
		    !completion_done(&dsi->bl_wait_completion)) {
	...
	} else if (!is_aod_exit && !(dsi->mi_cfg.aod_wait_frame)) {
...
static int mi_drm_update_fod_anim_status(struct mtk_dsi *dsi,
					 bool fod_anim_flag)
{
	struct mi_dsi_panel_cfg *mi_cfg = NULL;
...
#if IS_ENABLED(CONFIG_DRM_PANEL_N16U_42_02_0A_DSC_VDO)
	if (mi_cfg->fod_anim_flag == false && fod_anim_flag == true &&
	    dsi->mi_cfg.doze_state) {
		dsi->mi_cfg.dynamic_change_flag = true;
		DISP_INFO("exit aod\n");
		mi_disp_feature_event_notify_by_type(
			mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
			sizeof(dsi->mi_cfg.mdoze_brightness), DOZE_TO_NORMAL);
		dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] =
			DOZE_TO_NORMAL;
	} else if (mi_cfg->fod_anim_flag == true && fod_anim_flag == false &&
		   dsi->mi_cfg.doze_state && dsi->mi_cfg.user_doze_brightness) {
		dsi->mi_cfg.dynamic_change_flag = true;
		DISP_INFO("enter aod\n");
		mi_disp_feature_event_notify_by_type(
			mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
			sizeof(dsi->mi_cfg.mdoze_brightness),
			dsi->mi_cfg.mdoze_brightness + DOZE_BRIGHTNESS_MAX);
		dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] =
			dsi->mi_cfg.mdoze_brightness;
	}
#endif
...
int mi_drm_crtc_update_layer_state(struct drm_crtc *crtc)
{
	...
	cur_flags.fod_anim_flag =
		(mi_layer_flags & MI_LAYER_FOD_ANIM) ? true : false;
	mtk_drm_trace_c("%d|MI_LAYER_FOD_ANIM|%d", MI_TRACE_LAYER_ID,
			cur_flags.fod_anim_flag);
	...
	mtk_drm_trace_c("%d|MI_LAYER_AOD|%d", MI_TRACE_LAYER_ID,
			cur_flags.aod_flag);
	if (cur_flags.aod_flag != last_flags->aod_flag)
		DISP_INFO("layer AOD = %d, doze_state = %d\n",
			  cur_flags.aod_flag, doze_state);
#if IS_ENABLED(CONFIG_DRM_PANEL_N16U_42_02_0A_DSC_VDO)
	dsi->mi_cfg.doze_state = cur_flags.aod_flag;
#endif
