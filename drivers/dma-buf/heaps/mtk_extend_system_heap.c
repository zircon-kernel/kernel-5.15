...
static char *config_show_name[CONFIG_SHOW_MAX] = {
	"source_pool_free_order",	  "reserve_pool_used_order",
...
static char *served_process[POOL_PROCESS_NUMBER] = { "camerahalserver",
						     "hics.allocator@",
						     "phics.composer@" };
/* format: order 9 4 0 service time alloc time*/
static int g_reserve_config[MEM_TYPE_MAX][POOL_CONFIG_TYPE_MAX] = {
	{ 15, 3200, 38400, 100, 1000 },
	{ 15, 3200, 76800, 100, 1000 }
};
static int g_camera_reserve_config[MEM_TYPE_MAX][POOL_CONFIG_TYPE_MAX] = {
	{ 15, 3200, 51200, 100, 500 },
...
{
	return atomic_read(
		&reserve_extend_info.reserve_order[order_to_index(pool->order)]);
}
...
{
	return (atomic_read(&reserve_extend_info.reserve_order[order_to_index(
			pool->order)]) *
		DYNAMIC_POOL_LOW_MARK_PERCENT) /
	       100;
}
 ...
 */
static bool __dynamic_pool_zone_watermark_ok(struct zone *z, unsigned int order,
					     unsigned long mark,
					     int highest_zoneidx,
					     long free_pages)
{
...
static bool dynamic_pool_zone_watermark_ok_safe(struct zone *z,
						unsigned int order,
						unsigned long mark,
						int highest_zoneidx)
{
	...
	return __dynamic_pool_zone_watermark_ok(z, order, mark, highest_zoneidx,
						free_pages);
}
...
static bool dynamic_pool_refill_ok(struct dynamic_page_pool *pool)
{
	...
	for (i = classzone_idx; i >= 0; i--) {
		...
		mark += 1 << pool->order;
		if (!dynamic_pool_zone_watermark_ok_safe(zone, pool->order,
							 mark, classzone_idx)) {
...
static void dynamic_page_pool_refill(struct dynamic_page_pool *pool)
{
	...
	while (reserve_extend_info.use_reserve_pool &&
	       !dynamic_pool_fillmark_reached(pool) &&
	       dynamic_pool_refill_ok(pool)) {
		struct page *page;
		struct reserve_extend *p_re;
		p_re = &reserve_extend_info;
		if (!refilled) {
			atomic_inc(&p_re->refill_count[order_to_index(
				pool->order)]);
			atomic_set(&p_re->refill_amount_last[order_to_index(
					   pool->order)],
				   0);
			refilled = true;
		}
		...
		if (!page)
			break;
		atomic_inc(
			&p_re->refill_amount_last[order_to_index(pool->order)]);
...
static void reserve_page_pool_free(struct dynamic_page_pool **reserve_pool)
{
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		if (reserve_extend_info.debug & CLOSE_RESERVE_POOL)
			pr_info("reserve pool free %d order %d", freed,
				pool->order);
	}
}
	...
	for (i = 0; i < CONFIG_SHOW_MAX; i++) {
		...
		for (j = 0; j < NUM_ORDERS; j++) {
			...
			if (i == SOURCE_POOL_FREE_ORDER ||
				...
				val = config_show_pool[i][j]
					      ->count[POOL_LOWPAGE] +
				      config_show_pool[i][j]
					      ->count[POOL_HIGHPAGE];
			else
			...
			char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d %d %d cmp:%d\n",
			 pool_pid[COMPOSER], pool_pid[ALLOCATOR],
			 pool_pid[RESERVE_PROVIDER], use_name_cmp);
}
...
static bool update_pool_pid(int new_pid)
{
	...
	for (i = 0; i < POOL_PROCESS_NUMBER; i++) {
		...
		if (!task) {
			rcu_read_unlock();
			pr_info("no find pid COV! %d -> %d", new_pid,
				pool_pid[i]);
			pool_pid[i] = new_pid;
			new_pid_index = i;
			continue;
			 ...
			 const char *buf, size_t len)
{
	...
	if (pid > 0) {
		...
		pr_info("caller pid %d %s ppid %d", current->pid, current->comm,
			ppid);
		if (pool_pid_usable_index < POOL_PROCESS_NUMBER) {
			pool_pid[pool_pid_usable_index++] = pid;
			pr_info("pid %d %s %d %s %d", pid, task->comm,
				task->group_leader->pid,
				task->group_leader->comm,
				pool_pid_usable_index);
		} else {
			pr_info("pid OVF! %d %s %d", pid,
				task->group_leader->comm,
				pool_pid_usable_index);
			if (!update_pool_pid(pid))
				pr_info("warning pid %d %s %d %s %d", pid,
					task->comm, task->group_leader->pid,
					task->group_leader->comm,
					pool_pid_usable_index);
		}
...
static ssize_t scene_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", reserve_extend_info.scene);
}

static ssize_t scene_store(struct kobject *kobj, struct kobj_attribute *attr,
			   const char *buf, size_t len)
{
	int scene, ret;
...
static struct attribute *reserve_pool_attrs[] = {
	&kobj_attr_enable.attr, &kobj_attr_debug.attr, &kobj_attr_config.attr,
	&kobj_attr_pid.attr,	&kobj_attr_scene.attr, NULL,
};
...
static void reserve_extend_info_init(struct dynamic_page_pool **global_pools)
{
	...
	config_show_val[RESERVE_POOL_USED_ORDER] =
		reserve_extend_info.use_order;
	config_show_val[RESERVE_POOL_RESERVE_ORDER] =
		reserve_extend_info.reserve_order;
	config_show_val[RESERVE_POOL_REFILL_COUNT] =
		reserve_extend_info.refill_count;
	config_show_val[RESERVE_POOL_REFILL_AMOUNT] =
		reserve_extend_info.refill_amount;
	config_show_val[RESERVE_POOL_REFILL_LAST_AMOUNT] =
		reserve_extend_info.refill_amount_last;
}
...
static void reserve_pool_sysfs_create(void)
{
	int err;
	struct kobject *kobj =
		kobject_create_and_add("reserve_pool", kernel_kobj);
...
void mtk_sys_heap_reserve_pool_init(struct dynamic_page_pool **global_pools)
{
	...
	struct sched_attr attr = { .sched_nice =
					   DYNAMIC_POOL_KTHREAD_NICE_VAL };
	int ret = -ENOMEM;
	int i;
	set_mem_type_index();

	for (i = 0; i < NUM_ORDERS; i++) {
		g_reserve_pools[i] = dynamic_page_pool_create(order_flags[i],
							      orders[i], true);
	...
	if (IS_ENABLED(CONFIG_MTK_DMABUF_HEAPS_PAGE_POOL)) {
		...
		if (IS_ERR(refill_worker)) {
			...
			if (ret) {
				...
				int i;
				for (i = 0; i < NUM_ORDERS; i++)
					g_reserve_pools[i]->refill_worker =
						refill_worker;
			}
		}
	}
...
static is_server_app(struct dynamic_page_pool **reserve_pools)
{
	...
	if (reserve_pools != NULL &&
	    current->signal->oom_score_adj == NATIVE_ADJ) {
		int j;
...
static inline void pr_info_normal(char *tag, struct page *page, bool served_app,
				  struct dynamic_page_pool *spool,
				  struct dynamic_page_pool *reserve_pool)
{
	...
	pr_info("%s: page %p %s["
		...
		current->pid, spool->order, spool_high_count, spool_low_count,
		reserve_pool_high_count, reserve_pool_low_count, served_app);
}

static inline void
pr_info_with_alloc_info(char *tag, struct page *page, bool served_app,
			struct dynamic_page_pool *spool,
			struct dynamic_page_pool *reserve_pool, s64 delta,
			gfp_t *gfp_mask)
{
	int spool_high_count;
	int spool_low_count;
...
get_page_from_reserve_pool(bool served_app, struct dynamic_page_pool *spool,
			   struct dynamic_page_pool *reserve_pool)
{
	struct page *page = NULL;
	...
	if (!reserve_pool_empty() && served_app) {
		...
		if (!high_order)
			start = atomic64_read(
				&reserve_extend_info.use_reserve_pool_start);
		if (high_order || start != 0) {
			...
			if (delta <
				...
				atomic64_set(&reserve_extend_info
						      .use_reserve_pool_start,
					     0);
				if (reserve_extend_info.debug &
				    RESERVE_POOL_START)
					pr_info_normal(
						"used reserve pool timeout",
						page, served_app, spool,
						reserve_pool);
			}
		}
	}
...
static inline struct page *
get_high_order_page_from_buddy(bool served_app, struct dynamic_page_pool *spool,
			       struct dynamic_page_pool *reserve_pool)
{
	struct page *page = NULL;
...
static inline struct page *
get_page_from_buddy(bool served_app, struct dynamic_page_pool *spool,
		    struct dynamic_page_pool *reserve_pool)
{
	s64 delta;
	...
	if (served_app &&
		...
		atomic64_set(&reserve_extend_info.use_reserve_pool_start,
		...
		if (reserve_extend_info.debug & RESERVE_POOL_START)
			pr_info_with_alloc_info("reserve-sp", page, served_app,
						spool, reserve_pool, delta,
						&spool->gfp_mask);
	}
	if (reserve_extend_info.debug & ALL_ORDER_SYSTEM_ALLOC) {
		pr_info_with_alloc_info("All alloc", page, served_app, spool,
					reserve_pool, delta, &spool->gfp_mask);
	}
	if (spool->order == 0) {
		if (served_app)
...
static inline void
wakeup_refill_thread_by_wmark(bool served_app, struct dynamic_page_pool *spool,
			      struct dynamic_page_pool *reserve_pool)
{
	if (IS_ENABLED(CONFIG_MTK_DMABUF_HEAPS_PAGE_POOL) && spool->order &&
	...
	if (served_app && reserve_pool != NULL &&
	    reserve_pool->refill_worker != NULL) {
		if (IS_ENABLED(CONFIG_MTK_DMABUF_HEAPS_PAGE_POOL) &&
		    reserve_pool->order &&
...
struct page *mtk_extend_sys_heap_alloc_largest_available(
	struct dynamic_page_pool **global_pools, unsigned long size,
	unsigned int max_order)
{
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		if (!page)
			page = get_high_order_page_from_buddy(
				served_app, g_pools[i], reserve_pools[i]);
		if (!page)
			page = get_page_from_reserve_pool(
				served_app, g_pools[i], reserve_pools[i]);
		if (reserve_extend_info.debug & ALL_CALLER)
			pr_info_normal("All", page, served_app, g_pools[i],
				       reserve_pools[i]);
		if (!page) {
			page = get_page_from_buddy(served_app, g_pools[i],
						   reserve_pools[i]);
		}
		if (!page)
			continue;
		wakeup_refill_thread_by_wmark(served_app, g_pools[i],
					      reserve_pools[i]);
		return page;
	}
	return NULL;
